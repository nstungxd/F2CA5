...
Partial Class AdminSection7MainPage
...
#End Region
  ...
  if not ispostback then
    ...
    if EditAndSendBack then
      Dim intTheValue AS Integer = MyData.DBFactory.DB.GetDataField(Of Integer)("SELECT DropDownID FROM zz_admin_dropdowns where " & adminutility.PresetWhere(pageinfo))
...
Sub BackToParent(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs)
  ...
  Sub FollowRelationship(ByVal sender As Object, ByVal e As AdminUtility.RelationshipEventArgs) Handles ListControl.RelationshipEvent, EditControl.RelationshipEvent
    ...
    Dim childinfo As SectionPageInfo = PageInfo.GetChildInfo("admin_dropdowns.aspx")
    Dim url As String = Nothing

    Select Case id
      Case 18
        url = "admin_dropdown_translations.aspx"
        childinfo.Presets.Item("ddt_dropdownid") = e.RowID
    End Select
