...
Partial Class AdminSection107List
...
Dim _UserFixedWhere As String
   ...
   Sub EditRecord(ByVal sender As Object, ByVal e As GridCommandEventArgs) Handles dg_TableList.EditCommand
       RaiseEvent EditRecordEvent(CType(dg_TableList.Items(e.Item.ItemIndex).GetDataKeyValue("admin_section_group_id"), Integer))
   End Sub
  ...
  End Sub
	...
	Sub doFormSubmit(source As Object, e As GridCommandEventArgs) Handles dg_TableList.ItemCommand 
	          Select Case e.Item.ItemType 
	               ...
	               Dim pkid as Integer = CType(dg_TableList.Items(e.Item.ItemIndex).GetDataKeyValue("admin_section_group_id"), Integer)
	               If e.CommandName = "Delete" Then 
	                    Dim strSQL As String = "update zz_admin_section_groups set admin_section_group_deleted = 1 where admin_section_group_id = " & pkid 
		            ...
		            ElseIf e.CommandName = "Move_Up" then 
						MyContent.PageContent.MoveUp("107", pkid)
                       BindList()
			        ElseIf e.CommandName = "Move_Down" then 
						    MyContent.PageContent.MoveDown("107", pkid)
                       BindList()
...
Public Sub BindList()
'#Sorting Order
   Dim strListOrderBy AS String = "admin_section_group_order"   

'#Define Fields and Tables to Display
Dim strSQL As String = "SELECT *, ROW_NUMBER() OVER (ORDER BY " & strListOrderBy & ") AS 'RowNumber'  FROM zz_admin_section_groups"
Dim strSQLCount As String = "SELECT COUNT(*) FROM zz_admin_section_groups"
Dim strWhere AS String = MyContent.PageContent.getWhereString(pnlSearchTable)  
     ...
     MyContent.PageContent.appendToWhere("admin_section_group_deleted = 0", strWhere) 
...
Sub Export(sender as object, e as EventArgs)
  Response.Redirect("custom-reports.aspx?id="&107)
End Sub
...
sub Page_PreRender(sender as Object, e as EventArgs) Handles Me.PreRender
  ...
  End If
End Sub
Protected Sub dg_TableList_RowDrop(ByVal sender As Object, ByVal e As GridDragDropEventArgs)
   If String.IsNullOrEmpty(e.HtmlElement) Then
       If e.DraggedItems(0).OwnerGridID = dg_TableList.ClientID Then
           
           '# Create DB connection instance
           Dim sdb As SqlDatabaseConnection = DBFactory.SqlDB
           
               '# Get the ID for the item we're moving & the ID for the item in the place we're moving too
           Dim itemID As Integer = CType(e.DraggedItems(0).GetDataKeyValue("admin_section_group_id"), Integer)
           Dim replacedItemID As Integer = CType(e.DestDataItem.GetDataKeyValue("admin_section_group_id"), Integer)
           
           '# Get priority for this item
           If Not IsDBNull(e.DraggedItems(0).GetDataKeyValue("admin_section_group_order")) AndAlso Not IsDBNull(e.DestDataItem.GetDataKeyValue("admin_section_group_order")) Then
               Dim itemOldPriority As Integer = CType(e.DraggedItems(0).GetDataKeyValue("admin_section_group_order"), Integer)
               Dim itemNewPriority As Integer = CType(e.DestDataItem.GetDataKeyValue("admin_section_group_order"), Integer)
               
               '# Update the list and set the order depending on the direction we're moving items
               If itemNewPriority > itemOldPriority Then
               
                   '# Moving down
                   DBFactory.DB.RunSQL("UPDATE zz_admin_section_groups SET admin_section_group_order = admin_section_group_order - 1 WHERE admin_section_group_order <= " & itemNewPriority.ToString() & " AND admin_section_group_id <> " & itemID.ToString() & " AND admin_section_group_order > " & itemOldPriority.ToString())
               
               ElseIf itemNewPriority < itemOldPriority Then
               ...
               DBFactory.DB.RunSQL("UPDATE zz_admin_section_groups SET admin_section_group_order = " & itemNewPriority.ToString() & " WHERE admin_section_group_id = " & itemID.ToString())
