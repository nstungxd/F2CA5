...
Partial Class AdminSection12List
...
Dim _UserFixedWhere As String
   ...
   Sub EditRecord(ByVal sender As Object, ByVal e As GridCommandEventArgs) Handles dg_TableList.EditCommand
       RaiseEvent EditRecordEvent(CType(dg_TableList.Items(e.Item.ItemIndex).GetDataKeyValue("admin_section_id"), Integer))
   End Sub
  ...
  End Sub
	...
	Sub doFormSubmit(source As Object, e As GridCommandEventArgs) Handles dg_TableList.ItemCommand 
	          Select Case e.Item.ItemType 
	               ...
	               Dim pkid as Integer = CType(dg_TableList.Items(e.Item.ItemIndex).GetDataKeyValue("admin_section_id"), Integer)
	               If e.CommandName = "Delete" Then 
	                    Dim strSQL As String = "DELETE FROM zz_admin_sections WHERE admin_section_id = " & pkid 
		            ...
		            ElseIf e.CommandName = "Move_Up" then 
						MyContent.PageContent.MoveUp("12", pkid)
                       BindList()
			        ElseIf e.CommandName = "Move_Down" then 
						    MyContent.PageContent.MoveDown("12", pkid)
                       BindList()
...
Public Sub BindList()
'#Sorting Order
   Dim strListOrderBy AS String = "admin_section_menu_order"   

'#Define Fields and Tables to Display
Dim strSQL As String = "SELECT *, ROW_NUMBER() OVER (ORDER BY " & strListOrderBy & ") AS 'RowNumber'  FROM zz_admin_sections"
Dim strSQLCount As String = "SELECT COUNT(*) FROM zz_admin_sections"
...
End Sub 
Protected Sub search_input_admin_section_group_fk_Init(sender as object, e as eventargs) handles search_input_admin_section_group_fk.Init
...
Sub Export(sender as object, e as EventArgs)
  Response.Redirect("custom-reports.aspx?id="&12)
End Sub
...
Protected Sub dg_TableList_RowDrop(ByVal sender As Object, ByVal e As GridDragDropEventArgs)
   If String.IsNullOrEmpty(e.HtmlElement) Then
       If e.DraggedItems(0).OwnerGridID = dg_TableList.ClientID Then
           ...
           Dim itemID As Integer = CType(e.DraggedItems(0).GetDataKeyValue("admin_section_id"), Integer)
           Dim replacedItemID As Integer = CType(e.DestDataItem.GetDataKeyValue("admin_section_id"), Integer)
           
           '# Get priority for this item
           If Not IsDBNull(e.DraggedItems(0).GetDataKeyValue("admin_section_menu_order")) AndAlso Not IsDBNull(e.DestDataItem.GetDataKeyValue("admin_section_menu_order")) Then
               Dim itemOldPriority As Integer = CType(e.DraggedItems(0).GetDataKeyValue("admin_section_menu_order"), Integer)
               Dim itemNewPriority As Integer = CType(e.DestDataItem.GetDataKeyValue("admin_section_menu_order"), Integer)
               
               '# Update the list and set the order depending on the direction we're moving items
               If itemNewPriority > itemOldPriority Then
               
                   '# Moving down
                   DBFactory.DB.RunSQL("UPDATE zz_admin_sections SET admin_section_menu_order = admin_section_menu_order - 1 WHERE admin_section_menu_order <= " & itemNewPriority.ToString() & " AND admin_section_id <> " & itemID.ToString() & " AND admin_section_menu_order > " & itemOldPriority.ToString())
               ...
               DBFactory.DB.RunSQL("UPDATE zz_admin_sections SET admin_section_menu_order = " & itemNewPriority.ToString() & " WHERE admin_section_id = " & itemID.ToString())
