...
Partial Class AdminSection14List
...
Dim _UserFixedWhere As String
   ...
   Sub EditRecord(ByVal sender As Object, ByVal e As GridCommandEventArgs) Handles dg_TableList.EditCommand
       RaiseEvent EditRecordEvent(CType(dg_TableList.Items(e.Item.ItemIndex).GetDataKeyValue("list_field_id"), Integer))
   End Sub
  ...
  End Sub
	...
	Sub doFormSubmit(source As Object, e As GridCommandEventArgs) Handles dg_TableList.ItemCommand 
	          Select Case e.Item.ItemType 
	               ...
	               Dim pkid as Integer = CType(dg_TableList.Items(e.Item.ItemIndex).GetDataKeyValue("list_field_id"), Integer)
	               If e.CommandName = "Delete" Then 
	                    Dim strSQL As String = "DELETE FROM zz_admin_list_fields WHERE list_field_id = " & pkid 
		            ...
		            ElseIf e.CommandName = "Move_Up" then 
						MyContent.PageContent.MoveUp("14", pkid)
                       BindList()
			        ElseIf e.CommandName = "Move_Down" then 
						    MyContent.PageContent.MoveDown("14", pkid)
                       BindList()
...
Public Sub BindList()
'#Sorting Order
   Dim strListOrderBy AS String = "list_field_name"   
   if not PageInfo.Sort & "x"="x" then strListOrderBy = PageInfo.Sort

'#Define Fields and Tables to Display
Dim strSQL As String = "SELECT *, ROW_NUMBER() OVER (ORDER BY " & strListOrderBy & ") AS 'RowNumber'  FROM zz_admin_list_fields"
Dim strSQLCount As String = "SELECT COUNT(*) FROM zz_admin_list_fields"
...
Sub Export(sender as object, e as EventArgs)
  Response.Redirect("custom-reports.aspx?id="&14)
End Sub
