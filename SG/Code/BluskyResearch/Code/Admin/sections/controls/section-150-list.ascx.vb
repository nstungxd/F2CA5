...
Partial Class AdminSection150List
...
Dim _UserFixedWhere As String
   ...
   Sub EditRecord(ByVal sender As Object, ByVal e As GridCommandEventArgs) Handles dg_TableList.EditCommand
       RaiseEvent EditRecordEvent(CType(dg_TableList.Items(e.Item.ItemIndex).GetDataKeyValue("SurveyID"), Integer))
   End Sub
  ...
  End Sub
	...
	Sub doFormSubmit(source As Object, e As GridCommandEventArgs) Handles dg_TableList.ItemCommand 
	          Select Case e.Item.ItemType 
	               ...
	               Dim pkid as Integer = CType(dg_TableList.Items(e.Item.ItemIndex).GetDataKeyValue("SurveyID"), Integer)
	               If e.CommandName = "Delete" Then 
	                    Dim strSQL As String = "update Surveys set Deleted = 1 where SurveyID = " & pkid 
	                    MyData.DBFactory.db.RunSQL(strSQL)
	                    ListView() 

elseif e.CommandName="toggle_ViewDetails" then 
    DBFactory.DB.RUnSQL("UPDATE Surveys SET ViewDetails=1 - ISNULL(ViewDetails, 0) WHERE SurveyID= " & pkid)
    ListView()
		            ...
		            ElseIf e.CommandName = "Move_Up" then 
						MyContent.PageContent.MoveUp("150", pkid)
                       BindList()
			        ElseIf e.CommandName = "Move_Down" then 
						    MyContent.PageContent.MoveDown("150", pkid)
                       BindList()
...
Public Sub BindList()
'#Sorting Order
   Dim strListOrderBy AS String = "SurveyID"   
   if not PageInfo.Sort & "x"="x" then strListOrderBy = PageInfo.Sort

'#Define Fields and Tables to Display
Dim strSQL As String = "SELECT *, ROW_NUMBER() OVER (ORDER BY " & strListOrderBy & ") AS 'RowNumber'  FROM Surveys"
Dim strSQLCount As String = "SELECT COUNT(*) FROM Surveys"
Dim strWhere AS String = MyContent.PageContent.getWhereString(pnlSearchTable)  
     ...
     MyContent.PageContent.appendToWhere("Deleted = 0", strWhere) 
...
Sub Export(sender as object, e as EventArgs)
  Response.Redirect("custom-reports.aspx?id="&150)
End Sub
