...
Partial Class AdminSection21Edit
Inherits PingCore.Controls.AdminSectionEditControl
Private Const SectionID as integer = 21
...
'   End Property
 ...
 if intRecordID = 0 then: edittitle.Text = "Adding New Field Group" : end if
       ...
       MyContent.PageContent.FocusFirstControl(pnl_edit)
		if intRecordID <>  0 then
			...
			Dim strSQL As String = "SELECT * FROM zz_admin_section_field_groups  WHERE  field_group_id='" & PageContent.addSlashes(PrimaryKeyValue) & "'"
			Dim dt As DataTable = MyData.DBFactory.DB.GetDataTable(strSQL)
           Dim Row As DataRow = dt.Rows(0)
      
      
prechanges_field_group_name.Text = MyContent.PageContent.MakeStr(Row.Item("field_group_name"))
input_field_group_name.Text = MyContent.PageContent.MakeStr(Row.Item("field_group_name"))
      
input_field_group_MaxAuthority.Text = MyContent.PageContent.MakeInt(Row.Item("field_group_MaxAuthority"))
prechanges_field_group_MaxAuthority.Text = MyContent.PageContent.MakeInt(Row.Item("field_group_MaxAuthority"))
           Row = Nothing
           dt = Nothing
		else
...
input_field_group_name.Text =  ""
      
input_field_group_MaxAuthority.Text = ""
prechanges_field_group_MaxAuthority.Text = ""
		end if
		page1.Visible = True
		currpage.Text=1
...
' Add/Update info for field field_group_id

' Add/Update info for field field_group_name
 strPageAddNewFields &= "field_group_name,"
 strPageAddNewValues &= "'" &  MyContent.PageContent.addSlashes(MyContent.PageContent.makeStr(input_field_group_name.Text)) & "',"

 strPageUpdateString &=  "field_group_name='" &  MyContent.PageContent.addSlashes(MyContent.PageContent.makeStr(input_field_group_name.Text)) & "',"

' Add/Update info for field field_group_MaxAuthority
strPageAddNewFields &= "field_group_MaxAuthority,"
strPageAddNewValues &= MyContent.PageContent.MakeInt(input_field_group_MaxAuthority.Text) & ","

strPageUpdateString &= "field_group_MaxAuthority=" & MyContent.PageContent.MakeInt(input_field_group_MaxAuthority.Text) & ","
   ...
   strPageAddNewString = "INSERT INTO zz_admin_section_field_groups (" & strPageAddNewFields & ") values (" & strPageAddNewValues & ");"
	strPageUpdateString = "UPDATE zz_admin_section_field_groups SET " &  left(strPageUpdateString, len(strPageUpdateString )-1) & " WHERE  field_group_id='" & PageContent.addSlashes(PrimaryKeyValue) & "'" & ";"    
