<%@ Control Language="VB" AutoEventWireup="false"   Inherits="AdminSection21List" CodeFile="section-21-list.ascx.vb" %>
...
<header>
<h3>A Field Groups List</h3>
   <% if CanAdd then %>
       <asp:linkbutton runat='Server' id='add_new_link1' CssClass='admin_links btn_addnew' onCommand='AddNew'>New Field Group</asp:linkbutton>
       <% if HasParent then %>
...
<MasterTableView DataKeyNames="field_group_id"> 
<Columns> 
<telerik:GridBoundColumn ItemStyle-CssClass='css_field_group_name' SortExpression='field_group_name' HeaderText='Name' DataField='field_group_name'   />
