<%@ Control Language="VB" AutoEventWireup="false"   Inherits="AdminSection28List" CodeFile="section-28-list.ascx.vb" %>
...
<header>
<h3>A S Dependent Fields List</h3>
   <% if CanAdd then %>
       <asp:linkbutton runat='Server' id='add_new_link1' CssClass='admin_links btn_addnew' onCommand='AddNew'>New Dependent Field</asp:linkbutton>
       <% if HasParent then %>
...
<MasterTableView DataKeyNames="df_id"> 
<Columns> 
<telerik:GridBoundColumn ItemStyle-CssClass='css_df_section_id' SortExpression='df_section_id' HeaderText='Section' DataField='df_section_id'   />
<telerik:GridBoundColumn ItemStyle-CssClass='css_df_desc' SortExpression='df_desc' HeaderText='Description' DataField='df_desc'   />
