...
Partial Class AdminSection29Edit
Inherits PingCore.Controls.AdminSectionEditControl
Private Const SectionID as integer = 29
...
'   End Property
 ...
 if intRecordID = 0 then: edittitle.Text = "Adding New Type of User" : end if
       ...
       MyContent.PageContent.FocusFirstControl(pnl_edit)
		if intRecordID <>  0 then
			...
			Dim strSQL As String = "SELECT * FROM tbl_usertypes  WHERE  utype_id='" & PageContent.addSlashes(PrimaryKeyValue) & "'"
			Dim dt As DataTable = MyData.DBFactory.DB.GetDataTable(strSQL)
           Dim Row As DataRow = dt.Rows(0)
      
      
prechanges_utype_name.Text = MyContent.PageContent.MakeStr(Row.Item("utype_name"))
input_utype_name.Text = MyContent.PageContent.MakeStr(Row.Item("utype_name"))
      
prechanges_utype_hasglobalaccess.Text =  MyContent.PageContent.dbBool(Row.Item("utype_hasglobalaccess"))
input_utype_hasglobalaccess.checked =  MyContent.PageContent.dbBool(Row.Item("utype_hasglobalaccess"))
           Row = Nothing
           dt = Nothing
		else
...
input_utype_name.Text =  ""
      
input_utype_hasglobalaccess.checked =  false
		end if
		page1.Visible = True
		currpage.Text=1
...
' Add/Update info for field utype_id

' Add/Update info for field utype_name
 strPageAddNewFields &= "utype_name,"
 strPageAddNewValues &= "'" &  MyContent.PageContent.addSlashes(MyContent.PageContent.makeStr(input_utype_name.Text)) & "',"

 strPageUpdateString &=  "utype_name='" &  MyContent.PageContent.addSlashes(MyContent.PageContent.makeStr(input_utype_name.Text)) & "',"

' Add/Update info for field utype_hasglobalaccess
 strPageAddNewFields &=  "utype_hasglobalaccess,"
 strPageAddNewValues &=   MyContent.PageContent.dbBool(input_utype_hasglobalaccess.checked) & ","


 strPageUpdateString &= "utype_hasglobalaccess=" &  MyContent.PageContent.dbBool(input_utype_hasglobalaccess.checked) & ","
   ...
   strPageAddNewString = "INSERT INTO tbl_usertypes (" & strPageAddNewFields & ") values (" & strPageAddNewValues & ");"
	strPageUpdateString = "UPDATE tbl_usertypes SET " &  left(strPageUpdateString, len(strPageUpdateString )-1) & " WHERE  utype_id='" & PageContent.addSlashes(PrimaryKeyValue) & "'" & ";"    
