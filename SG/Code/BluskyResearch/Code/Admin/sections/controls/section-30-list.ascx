<%@ Control Language="VB" AutoEventWireup="false"   Inherits="AdminSection30List" CodeFile="section-30-list.ascx.vb" %>
...
<header>
<h3>A Filters List</h3>
   <% if CanAdd then %>
       <asp:linkbutton runat='Server' id='add_new_link1' CssClass='admin_links btn_addnew' onCommand='AddNew'>New Admin Filter</asp:linkbutton>
       <% if HasParent then %>
...
<MasterTableView DataKeyNames="admin_section_filter_id"> 
<Columns> 
<telerik:GridBoundColumn ItemStyle-CssClass='css_admin_section_filter_id' SortExpression='admin_section_filter_id' HeaderText='ID' DataField='admin_section_filter_id'   />
<telerik:GridBoundColumn ItemStyle-CssClass='css_filter_field' SortExpression='filter_field' HeaderText='Field' DataField='filter_field'   />
