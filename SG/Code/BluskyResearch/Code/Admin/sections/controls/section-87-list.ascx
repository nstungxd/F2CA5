<%@ Control Language="VB" AutoEventWireup="false"   Inherits="AdminSection87List" CodeFile="section-87-list.ascx.vb" %>
...
<header>
<h3>A Custom Controls List</h3>
   <% if CanAdd then %>
       <asp:linkbutton runat='Server' id='add_new_link1' CssClass='admin_links btn_addnew' onCommand='AddNew'>New Custom Control</asp:linkbutton>
       <% if HasParent then %>
...
<MasterTableView DataKeyNames="sc_id"> 
<Columns> 
<telerik:GridBoundColumn ItemStyle-CssClass='css_sc_name' SortExpression='sc_name' HeaderText='Name' DataField='sc_name'   />
<telerik:GridBoundColumn ItemStyle-CssClass='css_sc_relpath' SortExpression='sc_relpath' HeaderText='Relative Path' DataField='sc_relpath'   />
