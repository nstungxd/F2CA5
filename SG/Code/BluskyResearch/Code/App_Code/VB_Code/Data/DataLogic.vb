...
#Region "tungns"
        Public Shared Function Access_GetSurveysByID(SurveyNumber As String) As Survey

            '# Database connection
            Dim dbc As SqlDatabaseConnection
            dbc = Data.DBConnection()

            Dim cmd As New SqlCommand()
            cmd.CommandText = "sg_Survey_GetByID"
            cmd.CommandType = CommandType.StoredProcedure

            cmd.Parameters.Add(dbc.NewParameter(Of String)("@SurveyID", SqlDbType.VarChar, SurveyNumber))

            Dim dt As DataTable = dbc.GetDataTableFromStoredProcedure(cmd)
            Dim result As New Survey()
            result.ID = Convert.ToString(dt.Rows(0).Item("ID"))
            result.Blockby = Convert.ToString(dt.Rows(0).Item("Blockby"))
            result.CreatedOn = Convert.ToDateTime(dt.Rows(0).Item("CreatedOn"))
            result.DecorativeHeaderImage = Convert.ToString(dt.Rows(0).Item("DecorativeHeaderImage"))
            result.ForwardOnly = Convert.ToString(dt.Rows(0).Item("ForwardOnly"))
            result.InternalTitle = Convert.ToString(dt.Rows(0).Item("InternalTitle"))
            result.Languages = Convert.ToString(dt.Rows(0).Item("Languages"))
            result.MinimumResponsesAllowed = Convert.ToString(dt.Rows(0).Item("MinimumResponsesAllowed"))
            result.ModifiedOn = Convert.ToDateTime(dt.Rows(0).Item("ModifiedOn"))
            result.QuestionsToHide = Convert.ToString(dt.Rows(0).Item("QuestionsToHide"))
            result.Status = Convert.ToString(dt.Rows(0).Item("Status"))
            result.SubType = Convert.ToString(dt.Rows(0).Item("SubType"))
            result.Team = Convert.ToString(dt.Rows(0).Item("Team"))
            result.Theme = Convert.ToString(dt.Rows(0).Item("Theme"))
            result.Title = Convert.ToString(dt.Rows(0).Item("Title"))
            result.TitleML = Convert.ToString(dt.Rows(0).Item("TitleML"))
            result.Type = Convert.ToString(dt.Rows(0).Item("Type"))
            result.ViewDetails = Convert.ToBoolean(dt.Rows(0).Item("ViewDetails"))
