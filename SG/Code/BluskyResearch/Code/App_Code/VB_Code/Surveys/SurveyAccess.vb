...
#End Region
#Region "tungns - test new class"
        Public Shared Function AccessGetSurveyByID(SurveyID As Integer, Optional SkipFiltering As Boolean = False) As SurveyLibrary.Survey

            '''''''''''''''''''''''''''''All response'''''''''''''''''
            Dim responseQues As New List(Of SurveyResponseQuestion)()
            responseQues.Add(New SurveyResponseQuestion() With { _
                .AnswerValue = "Option 1", _
                .FreeTextAnswer = True, _
                .OptionID = "", _
                .QuestionID = "2", _
                .QuestionShown = True _
            })
            ...
            Dim surveyRespose As New List(Of SurveyResponse)()
            surveyRespose.Add(New SurveyResponse() With { _
              .ContactID = "", _
                .DateSubmitted = Date.Now, _
                .ID = "12", _
                .IsTestData = False, _
                .ResponseComment = "", _
                .ResponseID = "12", _
                .ResponseQuestions = responseQues, _
                .Status = "Complete" _
            })
            ''''''''''''''''''''''''End All response''''''''''''''''''''''''''''''''
            Dim link As New List(Of KeyValuePair(Of String, String))
            link.Add(New KeyValuePair(Of String, String)("edit", "https://app.surveygizmo.com/login/v1?authenticate=b811c4e2133c98b8013e0c5862969f9190827b7d&account_user_id=308179&r=%2Fprojects%2Feditor%3Fid%3D2115604&signature=574465"))
            link.Add(New KeyValuePair(Of String, String)("publish", "https://app.surveygizmo.com/login/v1?authenticate=b811c4e2133c98b8013e0c5862969f9190827b7d&account_user_id=308179&r=%2Fdistribute%2Fpromote%3Fid%3D2115604&signature=3f97f2"))
            link.Add(New KeyValuePair(Of String, String)("campaign", "https://www.surveygizmo.com/s3/2115604/Survey-for-testing-3"))

            Dim surveyPage As New List(Of SurveyPage)()

            Dim surveyRespose1 As New List(Of SurveyResponse)()
            surveyRespose1.Add(New SurveyResponse() With { _
              .ContactID = "", _
                .DateSubmitted = Date.Now, _
                .ID = "12", _
                .IsTestData = False, _
            ...
            ''''''''''''''''''''''''''''QUESTION''''''''''''''''''''''''
            Dim surveyOption As New List(Of SurveyOption)()
            surveyOption.Add(New SurveyOption() With { _
                               .ID = "10001", _
                               .QuestionID = 2, _
                               .QuestionTitle = "aaa", _
                               .Title = "Option 1", _
            ...
            properties.Add(New KeyValuePair(Of String, String)("option_sort", ""))
            properties.Add(New KeyValuePair(Of String, String)("required", ""))
            properties.Add(New KeyValuePair(Of String, String)("hidden", ""))
            properties.Add(New KeyValuePair(Of String, String)("orientation", "VERT"))
            properties.Add(New KeyValuePair(Of String, String)("labels_right", "1"))
            properties.Add(New KeyValuePair(Of String, String)("map_key", "radio"))
            properties.Add(New KeyValuePair(Of String, String)("show_title", ""))
            properties.Add(New KeyValuePair(Of String, String)("question_description", ""))
            properties.Add(New KeyValuePair(Of String, String)("question_description_above", ""))
            properties.Add(New KeyValuePair(Of String, String)("softrequired", ""))
            properties.Add(New KeyValuePair(Of String, String)("disabled", ""))
            properties.Add(New KeyValuePair(Of String, String)("messages", ""))
            properties.Add(New KeyValuePair(Of String, String)("hide_after_response", ""))
            properties.Add(New KeyValuePair(Of String, String)("custom_css", ""))
            properties.Add(New KeyValuePair(Of String, String)("break_after", ""))
            properties.Add(New KeyValuePair(Of String, String)("url", "http://"))

            Dim surveyQuestion As New List(Of SurveyQuestion)()
            surveyQuestion.Add(New SurveyQuestion() With { _
                               .Comment = False, _
                               .HasShowhideDeps = False, _
                               .ID = 2, _
                               .Options = surveyOption, _
                               .Properties = properties, _
                               .Shortname = "", _
                               .SubQuestions = Nothing, _
                                .SubType = "radio", _
                               .Title = "aaa", _
                                .Type = "SurveyQuestion", _
                            .Varname = "" _
             })
            '''''''''''''''''''''''''''''''''''''''''''''''

            Dim test As New Survey()
            test.AllResponses = surveyRespose
            test.Blockby = ""
            test.CreatedOn = DateTime.Now
            test.DecorativeHeaderImage = ""
            test.ForwardOnly = False
            test.ID = "2115604"
            test.InternalTitle = "Survey for testing 3"
            test.Languages = "English"
            test.Links = link
            test.MinimumResponsesAllowed = 0
            test.ModifiedOn = Date.Now
            test.Questions = surveyQuestion
            test.QuestionsToHide = Nothing
            test.Responses = surveyRespose1
            test.Status = "Launched"
            test.SubType = "Standard Survey"
            test.Team = "533149"
            test.Theme = "80530"
            test.Title = "Survey for testing 3"
            test.TitleML = "Survey for testing 3"
            test.Type = "Survey"
            test.ViewDetails = True

            Dim dt As Survey = DataLogic.Access_GetSurveysByID("2115604")
            Return test
        End Function
#End Region
    End Class
