...
#Region "Page Methods"
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        ...
        'If (Request.Url.Host.ToLower.StartsWith("www.") OrElse Request.Url.Host.ToLower.StartsWith("admin")) Then
        '    Redirect301("admin.bluskyresearch.com")
        'End If
...
#Region "WWW Redirect"
    Protected Sub WWWRedirect()
        ...
        If Not (Request.Url.Host.ToLower.StartsWith("www.") OrElse Request.Url.Host.ToLower.StartsWith("localhost") OrElse Request.Url.Host.ToLower.StartsWith("uat") OrElse Request.Url.Host.ToLower.StartsWith("login") OrElse Request.Url.Host.ToLower.StartsWith("admin")) Then
...
#Region "Account/Log in/Logged in views"
    Protected Sub CheckLoggedInStatus()
        If UserLoggedIn() Then
            ...
            pnlAdminLnk.Visible = False

            '# Set user details
            If IdentityControl.Identity IsNot Nothing Then
                With IdentityControl.Identity
                    lblScreenName.Text = "You are currently logged in as: " & .ScreenName & " (" & .EmailAddress & ")"

                    If .UTypeID < 3 Then
                        '# Link for admin
                        pnlAdminLnk.Visible = True
                    End If
                End With
            End If
...
#Region "Global Content"
    Protected Sub LoadGlobalContent()
        ...
        If Not String.IsNullOrEmpty(SiteName) AndAlso SiteName <> "blank" Then
