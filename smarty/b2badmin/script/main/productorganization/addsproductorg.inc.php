...
if(count($_POST) > 0) {
   ...
   if($view == 'edit') {
      $arr = $sProductOrgObj->select($iProductId);
      //prints($arr); exit;
   } else {
      $view = "add";
   }
...
<tr>
   <td  valign="top"><span class="reqmsg" style="float:right;"><?php if(isset($arr[0]['var_msg'])){ if($arr[0]['var_msg'] != '') { echo $arr[0]['var_msg']; } } ?></span>
      <table width="100%" border="0" class="table-border" cellspacing="0" cellpadding="0">
         <tr>
            <td class="heading">Supplier Product Organization Information</td>
         </tr>
         <tr>
            <td valign="top">
   ...
   <td valign="top" align="center">
	<!-- onclick="checkvalid(this.form);return false;" -->
     ...
     <input type="image"  id="btnSave" alt="Save" title="Save" style="cursor:pointer" onclick="return submitFrm();" src="<?php  echo  ADMIN_IMAGES?>btn-save.gif" tabIndex="17">
      ...
      <img style="cursor:pointer" alt="Cancel" title="Cancel" src="<?php  echo  ADMIN_IMAGES?>btn-cancle.gif" onclick="return RedirectURL('index.php?file=po-sproductorg&view=index&AX=Yes');" tabIndex="19" onblur="$('#vFirstName').focus();">
   </td>
</tr>
</table>
</form>
         ...
         remote:{
            ...
            data:{
               val:function() {
   				...
   				table:function() {
   					return "<?php echo PRJ_DB_PREFIX?>_sproduct_organization";
   				}
      		}
         }
      },
