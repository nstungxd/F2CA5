...
{
   var url = SITE_URL+"index.php?file=or-aj_getsuppliercombo";
   var pars = "&b2orgid="+$('#iBuyer2Id').val()+"&fid=iSupplierId&fnm=Data[iSupplierId]"+"&snm="+$('#bnm').val()+"&scd="+$('#bcd').val()+"&sid="+sid+"&cls=required&ea=&styl=width:300px;&whr=&fr=";
	// alert(url+pars); return false;
   $.ajax({url:url, data:pars, type:'get', dataType:'html', async:true, success: function(resp, status, jqxhr) {
      if($.trim(resp)!='') {
         $('#sprdt').html(resp);
