...
function getb2bprdtbassoclist(vl,pg,cursort)
{
     $('#updating').show();
	$('#mod').val(vl);
   ...
   $('#cursort').val(cursort);
	listb2bprdtbassocs(pg);
}

function listb2bprdtbassocs(pg)
{
	pars = "";
	vl = $('#mod').val();
	if(vl == 'all' || $.trim(vl) == '')
	...
	{
		pars += "&mod=srch"+"&buyer2="+$.trim($('#buyer2').val())+"&product="+$.trim($('#product').val())+"&buyer="+$.trim($('#buyer').val())+"&code="+$.trim($('#code').val());
	}
	if(pg == 'prv')
	{
		pg = pg-1;
   ...
   if($('#cursorttype')){
	...
	url = SITE_URL+"index.php?file=or-aj_b2bprdtbassoclist";
	$('#mod').val(vl);
	// alert(url+pars);
...
function status(mode,val)
{
   ...
   url = SITE_URL+"index.php?file=or-aj_b2bprdtbassoclist_a";
	// alert(url+pars);
	$.post(url, pars, function(resp) {
      $('#updating').hide();
      $('#dispmsg').show();
