...
function getb2buyerassoclist(vl,pg,cursort)
{
     $('#updating').show();
	$('#mod').val(vl);
   ...
   $('#cursort').val(cursort);
	listb2buyerassocs(pg);
}

function listb2buyerassocs(pg)
{
	pars = "";
	vl = $('#mod').val();
	if(vl == 'all' || $.trim(vl) == '')
   ...
   if($('#cursorttype')){
	...
	url = SITE_URL+"index.php?file=or-aj_b2buyerasoclist";
	$('#mod').val(vl);
	// alert(url+pars);
...
function status(mode,val)
{
   ...
   url = SITE_URL+"index.php?file=or-aj_b2buyerassoclist_a";
	// alert(url+pars);
	$.post(url, pars, function(resp) {
      $('#updating').hide();
      $('#dispmsg').show();
