...
function getb2sprdtassoclist(vl,pg,cursort)
{
   $('#updating').show();
	$('#mod').val(vl);
   ...
   $('#cursort').val(cursort);
	listb2sprdtassocs(pg);
}

function listb2sprdtassocs(pg)
{
	pars = "";
	vl = $('#mod').val();
	if(vl == 'all' || $.trim(vl) == '')
   ...
   if($('#cursorttype')){
	...
	url = SITE_URL+"index.php?file=or-aj_b2sprdtassoclist";
	$('#mod').val(vl);
	// alert(url+pars);
...
function status(mode,val)
{
   ...
   url = SITE_URL+"index.php?file=or-aj_b2sprdtassoclist_a";
	// alert(url+pars);
	$.post(url, pars, function(resp) {
      $('#updating').hide();
      $('#dispmsg').show();
