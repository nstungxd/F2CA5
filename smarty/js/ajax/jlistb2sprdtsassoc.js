...
function getb2sprdtsassoclist(vl,pg,cursort)
{
	$('#updating').show();
	$('#mod').val(vl);
   ...
   $('#cursort').val(cursort);
	listb2sprdtsassocs(pg);
}

function listb2sprdtsassocs(pg)
{
	pars = "";
	vl = $('#mod').val();
	if(vl == 'all' || $.trim(vl) == '')
	...
	{
		pars += "&mod=srch"+"&buyer2="+$.trim($('#buyer2').val())+"&product="+$.trim($('#product').val())+"&supplier="+$.trim($('#supplier').val())+"&code="+$.trim($('#code').val());
	}
	if(pg == 'prv')
	{
		pg = pg-1;
   ...
   if($('#cursorttype')){
	...
	url = SITE_URL+"index.php?file=or-aj_b2sprdtsassoclist";
	$('#mod').val(vl);
	// alert(url+pars);
...
function status(mode,val)
{
   ...
   url = SITE_URL+"index.php?file=or-aj_b2sprdtsassoclist_a";
	// alert(url+pars);
	$.post(url, pars, function(resp) {
      $('#updating').hide();
      $('#dispmsg').show();
      ...
      getb2sprdtsassoclist('all',1);
	});
   // getb2bprdtbassoclist('all',1);
}
