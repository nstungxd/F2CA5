...
function listgroup(pg)
{
	...
	else if(vl == 'srch')
	{
		...
		pars += "&mod=srch"+"&vInvoiceNumber="+$.trim($('#vInvoiceNumber').val())+"&vInvoiceCode="+$.trim($('#vInvoiceCode').val())+"&buyername="+$.trim($('#buyername').val())+"&vSupplierName="+$.trim($('#vSupplierName').val())+"&fInvoiceTotal="+$.trim($('#fInvoiceTotal').val())+"&iNetPaymentDays="+$.trim($('#iNetPaymentDays').val())+"&fDate="+$.trim($('#fDate').val())+"&tDate="+$.trim($('#tDate').val())+"&status="+$.trim($('#status').val())+"";
	}
     pars += '&invStatus='+$('#invStatus').val();
	if(pg == 'prv')
	{
		pg = pg-1;
	}
   ...
   if($('#cursorttype')){
	...
	if($('#invsts')){
      pars += "&invsts="+$('#invsts').val();
   }
	url = SITE_URL+"index.php?file=u-aj_invacptlist";
	$('#mod').val(vl);
...
function Delete(mode,val)
{
     if(mode == 'deleteall' || mode == 'status') {
          var sel = 0;
          var orgsval = '';
          var orgs = document.getElementsByName('iInvoiceID[]');
     ...
     url = SITE_URL+"index.php?file=u-aj_invoicelist_a";
	// alert(url+pars);
	$.post(url, pars, function(resp)
	{
      $('#updating').hide();
