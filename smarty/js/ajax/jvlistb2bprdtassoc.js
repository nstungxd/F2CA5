...
function getb2bprdtassovlist(vl,pg,cursort)
{
   $('#updating').show();
	$('#mod').val(vl);
   ...
   $('#cursort').val(cursort);
	listb2bprdtassocsv(pg);
}

function listb2bprdtassocsv(pg)
{
	pars = "";
	vl = $('#mod').val();
	if(vl == 'all' || $.trim(vl) == '')
   ...
   if($('#cursorttype')){
	...
	url = SITE_URL+"index.php?file=or-aj_b2bprdtassovlist";
	$('#mod').val(vl);
	//alert(url+pars);
