function getb2bprdtbassovlist(vl,pg,cursort)
{
   $('#updating').show();
	$('#mod').val(vl);
   ...
   $('#cursort').val(cursort);
	listb2bprdtbassocsv(pg);
}

function listb2bprdtbassocsv(pg)
{
	pars = "";
	vl = $('#mod').val();
	if(vl == 'all' || $.trim(vl) == '')
   ...
   if($('#cursorttype')){
	...
	url = SITE_URL+"index.php?file=or-aj_b2bprdtbassovlist";
	$('#mod').val(vl);
	//alert(url+pars);
