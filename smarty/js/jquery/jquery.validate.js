...
$.validator = function( options, form ) {
	...
	$.data(this.currentForm,'prevoius_value','');
	this.init();
};
...
$.extend($.validator, {
	...
	methods: {
		...
		remote: function(value, element, param) {
			...
			// alert($.data(this.currentForm,'prevoius_value'));
			if (previous.old !== value || ! previous.valid || $.data(this.currentForm,'prevoius_value') != $(this.currentForm).serialize()) { //
				previous.old = value;
				var validator = this;
            ...
            //alert(param+data);
				...
				$.data(this.currentForm,'prevoius_value',$(this.currentForm).serialize());
				return "pending";
		...
		mindate: function(value, element, param)
		{
		   ...
		   vl[1] = vl[1] - 1;
			/*if(tvl.length>0) {
				var vdate = new Date(vl[0],vl[1],vl[2],tvl[0],tvl[1],tvl[2]);
			} else {*/
				var vdate = new Date(vl[0],vl[1],vl[2]);
			//}
		   ...
		   mvl[1] = mvl[1] - 1;
			var mdate = new Date(mvl[0],mvl[1],mvl[2]);
			// var mdate = new Date(mvl[0],mvl[1],mvl[2],mtvl[0],mtvl[1],mtvl[2]);
		...
		// check min limit of date
		mindatetime: function(value, element, param)
		{
		   ...
		   mvl[1] = mvl[1] - 1;
			...
			// alert(Math.round(vdate.getTime() - mdate.getTime())/(1000*3600));
			// alert(vdate.getTime()+'>'+mdate.getTime());
			if(parseFloat(param[1])>0) {
				// if((vdate.getTime() - mdate.getTime())/(1000*3600) >= parseFloat(param[1])) { 	//	for param as hours
				if((vdate.getTime() - mdate.getTime())/(1000*60) >= parseFloat(param[1])) {
					return true;
				} else {
					return false;
				}
