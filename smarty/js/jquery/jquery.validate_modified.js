...
$.extend($.validator, {
	...
	messages: {
      ...
      url: "Please enter a valid URL.",
		...
		decimals: $.validator.format("Number has wrong format."),
		ip: $.validator.format("IP has wrong format.")
	},
	...
	methods: {
		...
		dateformat: function(value, element) {
			return this.optional(element) || /^\d{2}[- \/.]\d{2}[- \/.]\d{4}$/.test(new Date(value));
		},
		// added from outside
		datetimeformat: function(value, element) {
			return this.optional(element) || /^\d{2}[- \/.]\d{2}[- \/.]\d{4}\s*?\d{2}[- :.]\d{2}$/.test(new Date(value));
		},
		...
		// added from outside
		zipus: function(value, element) {
			return this.optional(element) || /^(\d{5})(-\d{4})?$/.test(value);
		},
		// added from outside (http://igorgladkov.com/resources/demos/jquery.validator/demos/decimal_validation.html)
		zipCanada: function(value, element) {
			return this.optional(element) || /^[a-z][0-9][a-z]\s*?[0-9][a-z][0-9]$/i.test(value);
		},
