...
/*
* -------------------------------------------------------
* CLASSNAME:        Buyer2_Buyer_Association_ToVerify
* GENERATION DATE:  26.02.2011
* CLASS FILE:       /home/design/design/www/B2B/libraries/classes/application/class.Buyer2_Buyer_Association_ToVerify.php
* FOR MYSQL TABLE:  b2b_buyer2_buyer_association_toverify
* FOR MYSQL DB:     B2B
* -------------------------------------------------------
* AUTHOR:
...
class Buyer2_Buyer_Association_ToVerify
{
...
*/
	...
	function select($id)
	{
			if(($id > 0) && (trim($id) != ''))
			{
				$sql =  "SELECT * FROM ".PRJ_DB_PREFIX."_buyer2_buyer_association_toverify WHERE iVerifiedID = $id";
			} else {
				$sql =  "SELECT * FROM ".PRJ_DB_PREFIX."_buyer2_buyer_association_toverify WHERE iVerifiedID=$this->_iVerifiedID ";
		 }
...
*/
	...
	function delete($id)
	{
		if(trim($id)!='' && $id>0)
      {
		 $sql = "DELETE FROM ".PRJ_DB_PREFIX."_buyer2_buyer_association_toverify WHERE iVerifiedID = $id";
   ...
   */
   function del($where)
   {
      if(trim($where)!='')
      {
         $sql = "DELETE FROM ".PRJ_DB_PREFIX."_buyer2_buyer_association_toverify WHERE $where";
   		$result = $this->_obj->sql_query($sql);
         return $result;
      }
      return '';
...
*/
	...
	function insert($Data = Array())
	{
		 ...
		 $result = $this->_obj->MySQLQueryPerform(PRJ_DB_PREFIX."_buyer2_buyer_association_toverify",$Data,'insert');
		  return $result;	
	}
...
*/
	...
	function update($where)
	{
		 ...
		 $result = $this->_obj->MySQLQueryPerform(PRJ_DB_PREFIX."_buyer2_buyer_association_toverify",$Data,'update',$where);
		  return $result;	
   ...
   function getDetails($feild="*",$where="",$orderBy="",$groupBy="",$limit="")
	{
      ...
      $sql = "SELECT $feild FROM ".PRJ_DB_PREFIX."_buyer2_buyer_association_toverify b2bav $cnt $limit";
      $row = $this->_obj->MySQLSelect($sql);
      return $row;
   }
   ...
   function getJoinTableInfo($jtbl,$fields="*",$where="",$orderBy="",$groupBy="",$limit="",$pg="") 
   {
      ...
      $sql =  "SELECT $fields FROM ".PRJ_DB_PREFIX."_buyer2_buyer_association_toverify b2bav $jtbl $cnt $limit";
		// echo $sql; exit;
		$row = $this->_obj->MySQLSelect($sql);
		if($pg=="yes")
		{
			$sql_count =  "SELECT Count(*) as tot FROM ".PRJ_DB_PREFIX."_buyer2_buyer_association_toverify b2bav $jtbl $cnt_count";
   ...
   function chkRecVrf($ary=array())
   {
      ...
      if(!(is_array($ary) && count($ary)>0)) {
      ...
      } else if(! (($ary[0]['eStatus']=='Active' || $ary[0]['eStatus']=='Inactive') && $ary[0]['eNeedToVerify']!='Yes') && ($_SESSION['SESS_'.PRJ_CONST_PREFIX.'_ORGTYPE']=='Buyer2' || $_SESSION['SESS_'.PRJ_CONST_PREFIX.'_USRTYPE']=='SM')) {
         ...
         // pr($ary); exit;
         if($ary[0]['eNeedToVerify']=='Yes' && ($_SESSION['SESS_'.PRJ_CONST_PREFIX.'_ORGTYPE']=='Buyer2' || $_SESSION['SESS_'.PRJ_CONST_PREFIX.'_USRTYPE']=='SM')) {
            if($sess_usertype_short==$ary[0]['eModifiedBy'] && $sess_id==$ary[0]['iModifiedByID']) {
               $vsts = 'uhc';
            } else if($sess_usertype_short==$ary[0]['eModifiedBy'] || $sess_usertype=='securitymanager') {
               $vsts = 'ucv';
   ...
   function getHistory($id)
	{
		$sql = "Select * from ".PRJ_DB_PREFIX."_buyer2_buyer_association_toverify where iAssociationId=$id Order By iVerifiedID ASC ";
