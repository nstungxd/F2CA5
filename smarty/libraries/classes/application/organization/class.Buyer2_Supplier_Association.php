...
/*
* -------------------------------------------------------
* CLASSNAME:        Buyer2_Supplier_Association
* GENERATION DATE:  26.02.2011
* CLASS FILE:       /home/design/design/www/B2B/libraries/classes/application/class.Buyer2_Supplier_Association.php
* FOR MYSQL TABLE:  b2b_buyer2_supplier_association
* FOR MYSQL DB:     B2B
* -------------------------------------------------------
* AUTHOR:
...
class Buyer2_Supplier_Association
{
...
*/
	...
	function select($id)
	{
			if(($id > 0) && (trim($id) != ''))
			{
				$sql =  "SELECT * FROM ".PRJ_DB_PREFIX."_buyer2_supplier_association WHERE iAssociationId = $id";
			} else {
				$sql =  "SELECT * FROM ".PRJ_DB_PREFIX."_buyer2_supplier_association WHERE iAssociationId=$this->_iAssociationId ";
		 }
...
*/
	...
	function delete($id)
	{
	  if(trim($id)!='' && $id>0)
     {
		 $sql = "DELETE FROM ".PRJ_DB_PREFIX."_buyer2_supplier_association WHERE iAssociationId = $id";
   ...
   */
   function del($where)
   {
      if(trim($where)!='')
      {
         $sql = "DELETE FROM ".PRJ_DB_PREFIX."_buyer2_supplier_association WHERE $where";
   		$result = $this->_obj->sql_query($sql);
         return $result;
      }
      return '';
...
*/
	...
	function insert($Data = Array())
	{
		...
		if(!is_array($Data) || count($Data)<1) {
		 ...
		 $result = $this->_obj->MySQLQueryPerform(PRJ_DB_PREFIX."_buyer2_supplier_association",$Data,'insert');
		  return $result;
	}
...
*/
	...
	function update($where)
	{
		 ...
		 $result = $this->_obj->MySQLQueryPerform(PRJ_DB_PREFIX."_buyer2_supplier_association",$Data,'update',$where);
		  return $result;
   ...
   function getDetails($feild="*",$where="",$orderBy="",$groupBy="",$limit="")
	{
      ...
      $sql = "SELECT $feild FROM ".PRJ_DB_PREFIX."_buyer2_supplier_association b2sa $cnt $limit";
      $row = $this->_obj->MySQLSelect($sql);
      return $row;
   }
   ...
   function getJoinTableInfo($jtbl,$fields="*",$where="",$orderBy="",$groupBy="",$limit="",$pg="")
   {
      ...
      $sql =  "SELECT $fields FROM ".PRJ_DB_PREFIX."_buyer2_supplier_association b2sa $jtbl $cnt $limit";
		$row = $this->_obj->MySQLSelect($sql);
		if($pg=="yes")
		{
			$sql_count =  "SELECT Count(*) as tot FROM ".PRJ_DB_PREFIX."_buyer2_supplier_association b2sa $jtbl $cnt_count";
   ...
   function getUniqueCode($type)
	{
		$sql = "Select COUNT(*) as tot from ".PRJ_DB_PREFIX."_buyer2_supplier_association where dADate>='".date('Y-m-d')."'";
		// echo $sql; exit;
