...
* CLASS FILE:       /home/www/B2B/libraries/classes/application/class.OrganizationGroup.php
* FOR MYSQL TABLE:  b2b_organization_group
* FOR MYSQL DB:     B2B
* -------------------------------------------------------
* AUTHOR:
...
*/
	...
	protected $iGroupID;   // KEY ATTR. WITH AUTOINCREMENT
...
*/
	...
	function select($id)
	{
			if(($id > 0) && (trim($id) != ''))
			{
				$sql =  "SELECT * FROM ".PRJ_DB_PREFIX."_organization_group WHERE iGroupID = $id";
			} else {
				$sql =  "SELECT * FROM ".PRJ_DB_PREFIX."_organization_group WHERE iGroupID=$this->_iGroupID ";
		 }
...
*/
	...
	function delete($id)
	{
		 $sql = "DELETE FROM ".PRJ_DB_PREFIX."_organization_group WHERE iGroupID = $id";
...
*/
	...
	{
		 $this->_iGroupID = '';
		 $this->iGroupID = ""; // clear key for autoincrement
		 ...
		 $result = $this->_obj->MySQLQueryPerform(PRJ_DB_PREFIX."_organization_group",$Data,'insert');
		 return $result;
	}
...
*/
	...
	function update($where)
	{
		 ...
		 $result = $this->_obj->MySQLQueryPerform(PRJ_DB_PREFIX."_organization_group",$Data,'update',$where);
		 return $result;
	}
	...
	*/
     ...
     function getDetails($feild="*",$where="",$OrderBy="",$GroupBy="",$limit="") {
       ...
       $sql =  "SELECT $feild FROM ".PRJ_DB_PREFIX."_organization_group $cnt $groupby $orderby";
		 $row =  $this->_obj->MySQLSelect($sql);
       return $row;
	}
   ...
   function getJoinTableInfo($jtbl,$fields='*',$where='',$orderBy='',$groupBy='',$limit='',$pg='')
	{
	  ...
	  if($orderBy != '') {
		...
		$sql =  "SELECT $fields FROM ".PRJ_DB_PREFIX."_organization_group grp $jtbl $cnt $limit";
		$row = $this->_obj->MySQLSelect($sql);
		//echo $sql;exit;
		if($pg=='yes') {
			  $sql_count =  "SELECT Count(*) as tot FROM ".PRJ_DB_PREFIX."_organization_group grp $jtbl $cnt";
