...
*
* -------------------------------------------------------
* CLASSNAME:        Organization_Preference
* GENERATION DATE:  19.04.2010
* CLASS FILE:       /home/www/B2B/libraries/classes/application/class.Organization_Preference.php
* FOR MYSQL TABLE:  b2b_organization_default_settings
* FOR MYSQL DB:     B2B
* -------------------------------------------------------
* AUTHOR:
...
*/
	...
	function select($id)
	{
			if(($id > 0) && (trim($id) != ''))
			{
				$sql =  "SELECT * FROM ".PRJ_DB_PREFIX."_organization_default_settings WHERE iAdditionalInfoID = $id";
			} else {
				$sql =  "SELECT * FROM ".PRJ_DB_PREFIX."_organization_default_settings WHERE iAdditionalInfoID=$this->_iAdditionalInfoID ";
		 }
...
*/
	...
	function delete($id)
	{
		 $sql = "DELETE FROM ".PRJ_DB_PREFIX."_organization_default_settings WHERE iAdditionalInfoID = $id";
...
*/
	...
	function insert()
	{
		 ...
		 $result = $this->_obj->MySQLQueryPerform(PRJ_DB_PREFIX."_organization_default_settings",$Data,'insert');
		 return $result;
	}
...
*/
	...
	function update($where)
	{
		...
		$result = $this->_obj->MySQLQueryPerform(PRJ_DB_PREFIX."_organization_default_settings",$Data,'update',$where);
		return $result;
	...
	*/
     ...
     function getDetails($feild="*",$where="",$OrderBy="",$GroupBy="",$limit='') {
       ...
       $sql =  "SELECT $feild FROM b2b_organization_default_settings $cnt $limit";
       //print $sql;
		 $row =  $this->_obj->MySQLSelect($sql);
       return $row;
	}
	...
	function getStatusDetails($iOrgId,$typ='status')
	{
		global $generalobj;
		if($typ == 'acceptance') {
			$ordlvl = 'vOrderAcceptanceLevel';
			$invlvl = 'vInvoiceAcceptanceLevel';
		} else if($typ == 'status') {
			$ordlvl = 'vOrderStatusLevel';
			$invlvl = 'vInvoiceStatusLevel';
		}
		$sql =  "SELECT $ordlvl, $invlvl FROM ".PRJ_DB_PREFIX."_organization_default_settings where iOrganizationId=$iOrgId";
		// echo $sql; exit;
		$orgdt = $this->_obj->MySQLSelect($sql);
		// prints($orgdt);
		$postatus = $orgdt[0][$ordlvl];
		// prints($postatus); // exit;
		$invstatus = $orgdt[0][$invlvl];
		if($postatus != '') {
		...
		if($invstatus != '') {
			...
			sort($invstatus);
		} else { $invstatus = array(); }
		$dsql = "Select iStatusID from ".PRJ_DB_PREFIX."_status_master where vStatus_en IN ('Create','Issued','Accepted') AND eFor='PO' ";
		$podfl = $this->_obj->MySQLSelect($dsql);
		for($i=0;$i<count($podfl);$i++) {
			$postatus[] = $podfl[$i]['iStatusID'];
		}
		sort($postatus);
		$dsql = "Select iStatusID from ".PRJ_DB_PREFIX."_status_master where vStatus_en IN ('Create','Issued','Accepted') AND eFor='Invoice' ";
		$invdfl = $this->_obj->MySQLSelect($dsql);
		for($i=0;$i<count($invdfl);$i++) {
			$invstatus[] = $invdfl[$i]['iStatusID'];
		}
		sort($invstatus);
		$postatus = array_unique($postatus);
		$invstatus = array_unique($invstatus);
		//prints($postatus); exit;
		//prints($invstatus); exit;
		$lang = $_SESSION['SESS_'.PRJ_CONST_PREFIX.'_LANG'];
		$status_ary = array();
		for($l=0;$l<count($postatus);$l++) {
			if(isset($postatus[$l])) {
				$sql = "Select * from ".PRJ_DB_PREFIX."_status_master where iStatusID=$postatus[$l] ";
				$podt = $this->_obj->MySQLSelect($sql);
				$status_ary['po'][] = Array("vStatus" => $podt[0]["vStatus_".$lang], "vStatusMsg" => $podt[0]["vStatusMsg_".$lang], "iDisplayOrder" => $podt[0]['iDisplayOrder'], "iStatusID" => $postatus[$l]);
			}
		}
		for($l=0;$l<count($invstatus);$l++) {
			if(isset($invstatus[$l])) {
				$sql = "Select * from ".PRJ_DB_PREFIX."_status_master where iStatusID=$invstatus[$l] ";
				$invdt = $this->_obj->MySQLSelect($sql);
				$status_ary['inv'][] = Array("vStatus" => $invdt[0]["vStatus_".$lang], "vStatusMsg" => $invdt[0]["vStatusMsg_".$lang], "iDisplayOrder" => $invdt[0]['iDisplayOrder'], "iStatusID" => $invstatus[$l]); 	// $invstatus[$l]
			}
		}
		$status_ary['po'] = $generalobj->sortarray($status_ary['po'],'iDisplayOrder');
		$status_ary['inv'] = $generalobj->sortarray($status_ary['inv'],'iDisplayOrder');
		// prints($status_ary); exit;
		return $status_ary;
	}

	function getHistory($iOrgId)
	{
		$sql = "Select * from ".PRJ_DB_PREFIX."_organization_default_settings_toverify where iOrganizationID=$iOrgId ";
	...
	function getLastOrgPrefFor($orgid,$fld,$elast)
	{
		$dtls = $this->getDetails($fld," AND iOrganizationID=$orgid ");
		$lpermit = '';
		if(is_array($dtls) && count($dtls)>0 && isset($dtls[0][$fld]) && trim($dtls[0][$fld])!='')
		{
			if(strpos($dtls[0][$fld],',')!==false) {
				$permits = @ explode(',',$dtls[0][$fld]);
				sort($permits);
				foreach($permits as $ky => $vl) {
					if($vl<$elast) {
						$lpermit = $vl;
					}
				}
	...
	function getNextStatus($orgid,$crsts,$typ,$wname='')
	{
		$nsts = "";
		if($orgid>0 && $crsts>=0 && trim($typ)!='')
		{
			$opdtls = $this->getDetails("$typ", " AND iOrganizationID=$orgid ");
                        if(isset($opdtls[0][$typ]) && trim($opdtls[0][$typ])!='') {
				$orgp = @ explode(',', $opdtls[0][$typ]);                                
			}
                        $orgp = array_unique($orgp);
			$orgp = array_filter($orgp);                        
			if(count($orgp)>0) {
				foreach($orgp as $ky => $vl) {
					if($crsts < $vl) {
						$nsts = $vl;
						break;
					}
		...
		if($wname=='y' && trim($nsts)!='' && $nsts>0) {
			$sql = "Select vStatus_en from ".PRJ_DB_PREFIX."_status_master where iStatusID=$nsts";
			$snm = $this->_obj->MySQLSelect($sql);
			$nxsts[$nsts] = $snm[0]['vStatus_en'];
			return $nxsts;
		}
		return $nsts;
	}
