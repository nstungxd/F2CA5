...
/*
* -------------------------------------------------------
* CLASSNAME:        SProductOrganization
* GENERATION DATE:  25.01.2011
* CLASS FILE:       D:\wamp\www\new\B2B/libraries/classes/application/class.SProductOrganization.php
* FOR MYSQL TABLE:  b2b_sproduct_organization
* FOR MYSQL DB:     B2B
* -------------------------------------------------------
* AUTHOR:
...
*/
	...
	function select($id)
	{
			if(($id > 0) && (trim($id) != ''))
			{
				$sql =  "SELECT * FROM ".PRJ_DB_PREFIX."_sproduct_organization WHERE iProductId = $id";
			} else {
				$sql =  "SELECT * FROM ".PRJ_DB_PREFIX."_sproduct_organization WHERE iProductId=$this->_iProductId ";
		 }
...
*/
	...
	function delete($id)
	{
		 $sql = "DELETE FROM ".PRJ_DB_PREFIX."_sproduct_organization WHERE iProductId = $id";
...
*/
	...
	function insert()
	{
		 ...
		 $result = $this->_obj->MySQLQueryPerform(PRJ_DB_PREFIX."_sproduct_organization",$Data,'insert');
		  return $result;
	}
...
*/
	...
	function update($where)
	{
		 ...
		 $result = $this->_obj->MySQLQueryPerform(PRJ_DB_PREFIX."_sproduct_organization",$Data,'update',$where);
		  return $result;
   ...
   function getDetails($feild="*",$where="",$orderBy="",$groupBy="",$limit="")
	{
       ...
       $sql =  "SELECT $feild FROM ".PRJ_DB_PREFIX."_sproduct_organization spo $cnt $limit";
		 $row =  $this->_obj->MySQLSelect($sql);
       return $row;
	}
   ...
   function getJoinTableInfo($jtbl,$fields="*",$where="",$orderBy="",$groupBy="",$limit="",$pg="")
   {
      ...
      $sql =  "SELECT $fields FROM ".PRJ_DB_PREFIX."_sproduct_organization $jtbl $cnt $limit";
		$row = $this->_obj->MySQLSelect($sql);
		if($pg=="yes")
		{
			$sql_count =  "SELECT Count(*) as tot FROM ".PRJ_DB_PREFIX."_sproduct_organization spo $jtbl $cnt_count";
