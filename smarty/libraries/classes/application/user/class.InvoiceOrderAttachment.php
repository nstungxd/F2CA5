...
*
* -------------------------------------------------------
* CLASSNAME:        InvoiceOrderAttachment
* GENERATION DATE:  11.06.2010
* CLASS FILE:       /home/www/B2B/libraries/classes/application/class.InvoiceOrderAttachment.php
* FOR MYSQL TABLE:  b2b_invoice_order_attachment
* FOR MYSQL DB:     B2B
* -------------------------------------------------------
* AUTHOR:
...
*/
	...
	function select($id)
	{
			if(($id > 0) && (trim($id) != ''))
			{
				$sql =  "SELECT * FROM ".PRJ_DB_PREFIX."_invoice_order_attachment WHERE iAttachmentID = $id";
			} else {
				$sql =  "SELECT * FROM ".PRJ_DB_PREFIX."_invoice_order_attachment WHERE iAttachmentID=$this->_iAttachmentID ";
		 }
...
*/
	...
	function delete($id)
	{
		 $sql = "DELETE FROM ".PRJ_DB_PREFIX."_invoice_order_attachment WHERE iAttachmentID IN ($id) ";
...
*/
	...
	function insert()
	{
		 ...
		 $result = $this->_obj->MySQLQueryPerform(PRJ_DB_PREFIX."_invoice_order_attachment",$Data,'insert');
		  return $result;
	}
...
*/
	...
	function update($where)
	{
		 ...
		 $result = $this->_obj->MySQLQueryPerform(PRJ_DB_PREFIX."_invoice_order_attachment",$Data,'update',$where);
		  return $result;
   ...
   function getDetails($feild="*",$where="",$orderBy="",$groupBy="",$limit="")
	{
       ...
       $sql =  "SELECT $feild FROM ".PRJ_DB_PREFIX."_invoice_order_attachment $cnt $limit";
		 $row =  $this->_obj->MySQLSelect($sql);
       return $row;
	}
   ...
   function getJoinTableInfo($jtbl,$fields="*",$where="",$orderBy="",$groupBy="",$limit="",$pg="") {
      ...
      $sql =  "SELECT $fields FROM ".PRJ_DB_PREFIX."_invoice_order_attachment $jtbl $cnt $limit";
		$row = $this->_obj->MySQLSelect($sql);
		if($pg=="yes")
		{
			$sql_count =  "SELECT Count(*) as tot FROM ".PRJ_DB_PREFIX."_invoice_order_attachment $jtbl $cnt_count";
