...
*
* -------------------------------------------------------
* CLASSNAME:        PurchaseOrderHeading
* GENERATION DATE:  18.04.2012
* CLASS FILE:       /var/www/B2B/libraries/classes/application/class.PurchaseOrderHeading.php
...
*/
	...
	public function getvPoBuyerCode()
	{
		return $this->_vPoBuyerCode;
	}
	...
	public function geteRfq2Awarded()
	{
		return $this->_eRfq2Awarded;
	}
...
*/
	...
	public function setvPoBuyerCode($val)
	{
		 $this->_vPoBuyerCode =  $val;
	}
	...
	public function seteRfq2Awarded($val)
	{
		 $this->_eRfq2Awarded =  $val;
	}
...
*/
	...
	function select($id)
	{
		 ...
		 $this->_vPoBuyerCode = $row[0]['vPoBuyerCode'];
		 ...
		 $this->_eRfq2Awarded = $row[0]['eRfq2Awarded'];
...
*/
	...
	function insert($Data = Array())
	{
		 ...
		 $Data = array(
						...
						'vPoBuyerCode'		=>	$this->_vPoBuyerCode,
						...
						'vImage'		=>	$this->_vImage,
						'eRfq2Awarded'		=>	$this->_eRfq2Awarded,
						'eSaved'		=>	$this->_eSaved,
...
*/
   ...
   function getJoinTableInfo($jtbl,$fields="*",$where="",$orderBy="",$groupBy="",$limit="",$pg="")
	{
      ...
      $sql =  "SELECT $fields FROM ".PRJ_DB_PREFIX."_purchase_order_heading poh $jtbl $cnt $limit";
		// echo $sql; // exit;
		$row = $this->_obj->MySQLSelect($sql);
		if($pg=="yes")
		{
			$sql_count =  "SELECT Count(*) as tot FROM ".PRJ_DB_PREFIX."_purchase_order_heading poh $jtbl $cnt_count";
   ...
   function GetPurchaseOrderItems($iPurchaseOrderId) {
      $sql =  "SELECT * FROM ".PRJ_DB_PREFIX."_purchase_order_line WHERE iPurchaseOrderID = '".$iPurchaseOrderId."'";
		$row = $this->_obj->MySqlSelect($sql);
      // prints($row);exit;
      return $row;
   }
   ...
   function chkDuplicate($fileds,$data){
      ...
      $sql =  "SELECT iPurchaseOrderID as ID FROM ".PRJ_DB_PREFIX."_purchase_order_heading WHERE 1 $where";
		$row = $this->_obj->MySqlSelect($sql);
      //Prints($row);exit;
      if(count($row) > 0){
         $dup = $row[0][ID];
	...
	function getUniqueCode($type='po')
	{
		$sql = "Select COUNT(*) as tot from ".PRJ_DB_PREFIX."_purchase_order_heading where dCreateDate>'".date('Y-m-d')."'";
		// echo $sql; exit;
   ...
   function getHistory($iPoId,$orgid="0") 	// ,$orgid="0"
   {
		$oc = "";
		if($orgid>0) {
			$oc = " iOrganizationID=$orgid AND ";
		}
      $sql = "Select iVerifiedID,iItemID,eSubject,eType,vAction,vMailSubject_en,iOrganizationID,iCreatedBy,eCreatedType,eVerifiedBy,iVerifiedBy,dActionDate,dVerifyDate,vVerifyFromIP from ".PRJ_DB_PREFIX."_user_action_verification where $oc iOrganizationID=$orgid AND iItemID=$iPoId AND eSubject='PO' AND vMailSubject_en!='New purchase order waiting for acceptance' Order By dActionDate ASC "; 	// iOrganizationID=$orgid AND
      // echo $sql; exit;
      $pohdtls = $this->_obj->MySQLSelect($sql);
		for($l=0;$l<count($pohdtls);$l++)
		{
			$sql = "Select CONCAT(vFirstName,' ',vLastName) as name from ".PRJ_DB_PREFIX."_organization_user where iUserID=".$pohdtls[$l]['iCreatedBy'];
			$cusr = $this->_obj->MySQLSelect($sql);
			$pohdtls[$l]['createdby'] = $cusr[0]['name'];
		}
		return $pohdtls;
   }

   function chkinvex($ipoid)
   {
   	$sql =  "SELECT iInvoiceID FROM ".PRJ_DB_PREFIX."_inovice_order_heading WHERE iPurchaseOrderID=$ipoid";
		$row = $this->_obj->MySqlSelect($sql);
		if(is_array($row) && count($row)>0) { 	// && $row[0]['iInvoiceID']>0
			return 'n';
		} else {
			return 'y';
		}
   ...
   function getPurchaseOrderRfq2Buyer2OrgIds($id)
	{
		$sql = "Select iOrganizationID from ".PRJ_DB_PREFIX."_organization_master org INNER JOIN ".PRJ_DB_PREFIX."_rfq2_product_buyer2 rpb2 on org.iOrganizationID=rpb2.iBuyer2Id where rpb2.iRFQ2Id=(Select iRFQ2Id from ".PRJ_DB_PREFIX."_rfq2_master where iPurchaseOrderId=$id ORDER BY iRFQ2Id DESC LIMIT 0,1) ";
		//echo $sql; exit;
