...
class DBConnection
{
	...
	*/
	public function DBConnection($server="",$dbase="", $user="", $pass="")
	{
      ...
      $conn = mysql_connect($server,$user,$pass,false,65536);
      if(!$conn) {
		...
		$trs = mysql_query($tzsql,$conn); 	// UTC_TIMESTAMP() instead of NOW() & UTC_DATE() instead of CURDATE()
		$this->CONN = $conn;
		return true;
	}
	...
	*/
	public function MySQLSelect($sql="",$cached="")
	{
		...
		$chkurl2 = "";
		//print_r($_GET['file']);
		$chkfile = (isset($_GET['file']))? $_GET['file'] : '';
		//$chkfile = $_GET('file');
	...
	*/
	public function MySQLGetFields($table)
	{
		...
		$primarykey = (isset($primarykey))? $primarykey : '';
...
//// exit;
   ...
   // for executing sql statements in a single transaction
   function dotransaction($sqls=array())
   {
      $er = '';
