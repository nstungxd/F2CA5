...
{
   $sql1 = "UPDATE ".PRJ_DB_PREFIX."_buyer2_buyer_association SET eStatus = IF(eStatus='Inactive','Active','Inactive'), eNeedToVerify='Yes', eModifiedBy='".$sess_usertype_short."', iModifiedByID='".$sess_id."', dModifiedDate='".date('Y-m-d H:i:s')."', eVerifiedBy='', iVerifiedByID='', dVerifiedDate='', eRejectedBy='', iRejectedByID='', dRejectedDate='' WHERE iAssociationId IN ($vl)";
   $sql2 = "INSERT INTO b2b_buyer2_buyer_association_toverify
            (iAssociationId,iBuyer2Id,iBuyerId,vACode,vAccount1,vAcc1Code,vCurrency1,vAccount2,vAcc2Code,vCurrency2,dADate,vFromIP,iCreatedByID,eCreatedBy,iModifiedByID,eModifiedBy,dModifiedDate,iRejectedByID,eRejectedBy,dRejectedDate,iVerifiedByID,eVerifiedBy,dVerifiedDate,tReasonToReject,eNeedToVerify,eStatus)
            SELECT * FROM b2b_buyer2_buyer_association WHERE iAssociationId IN ($vl)";
   ...
   if($rs) {
      ...
      $esubtyp = 'Buyer2 Buyer Association Status Changed';
   ...
   $sql1 = "UPDATE ".PRJ_DB_PREFIX."_buyer2_buyer_association SET eStatus='Delete', eNeedToVerify='Yes', eModifiedBy='".$sess_usertype_short."', iModifiedByID='".$sess_id."', dModifiedDate='".date('Y-m-d H:i:s')."', eVerifiedBy='', iVerifiedByID='', dVerifiedDate='', eRejectedBy='', iRejectedByID='', dRejectedDate='' WHERE iAssociationId IN ($vl)";
   $sql2 = "INSERT INTO b2b_buyer2_buyer_association_toverify
            (iAssociationId,iBuyer2Id,iBuyerId,vACode,vAccount1,vAcc1Code,vCurrency1,vAccount2,vAcc2Code,vCurrency2,dADate,vFromIP,iCreatedByID,eCreatedBy,iModifiedByID,eModifiedBy,dModifiedDate,iRejectedByID,eRejectedBy,dRejectedDate,iVerifiedByID,eVerifiedBy,dVerifiedDate,tReasonToReject,eNeedToVerify,eStatus)
            SELECT * FROM b2b_buyer2_buyer_association WHERE iAssociationId IN ($vl)";
   // $rs = $dbobj->dotransaction($sqls);
   ...
   if($rs) {
      ...
      $esubtyp = 'Buyer2 Buyer Association Delete';
      $whr_cndt = " AND vType='$esubtyp' AND eSection='Member' ";
      $email_cnt = $emailObj->getDetails('*', $whr_cndt);
      $type = "Delete";
