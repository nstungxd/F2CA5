...
{
   $sql1 = "UPDATE ".PRJ_DB_PREFIX."_buyer2_sproduct_association SET eStatus = IF(eStatus='Inactive','Active','Inactive'), eNeedToVerify='Yes', eModifiedBy='".$sess_usertype_short."', iModifiedByID='".$sess_id."', dModifiedDate='".date('Y-m-d H:i:s')."', eVerifiedBy='', iVerifiedByID='', dVerifiedDate='', eRejectedBy='', iRejectedByID='', dRejectedDate='' WHERE iAssociationId IN ($vl)";
   $sql2 = "INSERT INTO b2b_buyer2_sproduct_association_toverify
            ...
            SELECT * FROM b2b_buyer2_sproduct_association WHERE iAssociationId IN ($vl)";
   ...
   if($rs) {
      ...
      $esubtyp = 'Buyer2 SProduct Association Status Changed';
   ...
   $sql1 = "UPDATE ".PRJ_DB_PREFIX."_buyer2_sproduct_association SET eStatus='Delete', eNeedToVerify='Yes', eModifiedBy='".$sess_usertype_short."', iModifiedByID='".$sess_id."', dModifiedDate='".date('Y-m-d H:i:s')."', eVerifiedBy='', iVerifiedByID='', dVerifiedDate='', eRejectedBy='', iRejectedByID='', dRejectedDate='' WHERE iAssociationId IN ($vl)";
   $sql2 = "INSERT INTO b2b_buyer2_sproduct_association_toverify
            (iAssociationId,iBuyer2Id,iProductId,vACode,fGlobalLimit,fOutstandingAmt,fFeePc,fFeeFlat,fAdvancePc,fMinValue,fMaxValue,vAccount1,vAccount2,vAccount3,vAccount4,vAccount5,vAccount6,vAccount7,vAccount8,vNarrative,dADate,vFromIP,iCreatedByID,eCreatedBy,iModifiedByID,eModifiedBy,dModifiedDate,iRejectedByID,eRejectedBy,dRejectedDate,iVerifiedByID,eVerifiedBy,dVerifiedDate,tReasonToReject,eNeedToVerify,eStatus)
            SELECT * FROM b2b_buyer2_sproduct_association WHERE iAssociationId IN ($vl)";
   // $rs = $dbobj->dotransaction($sqls);
   ...
   if($rs) {
      ...
      $esubtyp = 'Buyer2 SProduct Association Delete';
      $whr_cndt = " AND vType='$esubtyp' AND eSection='Member' ";
      $email_cnt = $emailObj->getDetails('*', $whr_cndt);
      $type = "Delete";
   ...
   for($l=0;$l<count($assocs);$l++)
   {
      if(isset($assocs[$l]['iAssociationId']) && $assocs[$l]['iAssociationId']>0) {
         if(is_array($email_cnt) && count($email_cnt)>0) {
            $verifyid = $b2spavObj->getDetails('iVerifiedID'," AND iAssociationId='".$assocs[$l]['iAssociationId']."' ",' iVerifiedID DESC ','',' LIMIT 0,1 ');
            if(isset($verifyid[0]['iVerifiedID']) && $verifyid[0]['iVerifiedID']>0) {
               ...
               $link = SITE_URL_DUM."b2sprodtasocview/".$assocs[$l]['iAssociationId'];
