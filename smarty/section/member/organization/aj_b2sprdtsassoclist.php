...
// include(S_SECTIONS."/member/memberaccess.php");
   ...
   if(!isset($assObj))
	{
		...
		$assObj = new Buyer2_Supplier_SProduct_Association();
	}

   $where = "";
   $_POST['srchval'] = (isset($_POST['srchval']))? $_POST['srchval'] : '';
	if($_POST['srchval']=='act') {
		$where .= " AND b2sps.eStatus='Active' ";
	} else if($_POST['srchval']=='inact') {
		$where .= " AND b2sps.eStatus='Inactive' ";
	}
	if($mode == 'all')
	{
		if($val != '') {
			$where .= " AND ((Select vCompanyName from ".PRJ_DB_PREFIX."_organization_master where iOrganizationID=b2sps.iBuyer2Id) LIKE '%$val%' OR
	...
	else if($mode == 'srch')
	{
      if(trim($buyer2) != '')
		...
		if(trim($code) != '')
		{
			$where .= " AND b2sps.vACode LIKE '%$code%'";
		}
      if(trim($product) != '')
		{
			$where .= " AND (Select vProductName from b2b_bproduct_organization where iProductId=b2sps.iProductId) LIKE '%$product%'";
		}
      if(trim($supplier) != '')
		{
	...
	if($_SESSION['SESS_'.PRJ_CONST_PREFIX.'_USER_TYPE_SHORT'] == 'OA' && $uorg_type=='Buyer2') {
		$where .= " AND (b2sps.iBuyer2ID='".$_SESSION['SESS_'.PRJ_CONST_PREFIX.'_ORGID']."')";
	} else if($_SESSION['SESS_'.PRJ_CONST_PREFIX.'_USER_TYPE_SHORT'] == 'OA' && $uorg_type!='Buyer') {
		$where .= " AND (b2sps.iSupplierId='".$_SESSION['SESS_'.PRJ_CONST_PREFIX.'_ORGID']."')";
	}
   ...
   if($cursort != '') {
      ...
      $orderBy = " b2sps.dADate DESC ";
   }
   ## ENDS HERE ###

   $where .= " AND NOT (b2sps.eStatus='Delete' AND b2sps.eNeedToVerify!='Yes') ";
	$limit = " LIMIT ".($page-1)*$REC_LIMIT_FRONT.", ".$REC_LIMIT_FRONT." ";
   $jtbl = "";
	$fields = "b2sps.*, (Select vCompanyName from ".PRJ_DB_PREFIX."_organization_master where iOrganizationID=b2sps.iBuyer2Id) as vBuyer2,
					 (Select vProductName from ".PRJ_DB_PREFIX."_bproduct_organization where iProductId=b2sps.iProductId) as vProduct,
	...
	$pgajxobj = new Paging($count,$page,"listb2sprdtsassocs",$REC_LIMIT_FRONT);
