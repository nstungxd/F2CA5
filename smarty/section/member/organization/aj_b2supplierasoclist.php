   ...
   if(!isset($assObj))
	{
		...
		$assObj = new Buyer2_Supplier_Association();
	}

   $where = "";
   $_POST['srchval'] = (isset($_POST['srchval']))? $_POST['srchval'] : '';
	if($_POST['srchval']=='act') {
		$where .= " AND b2sa.eStatus='Active' ";
	} else if($_POST['srchval']=='inact') {
		$where .= " AND b2sa.eStatus='Inactive' ";
	}
	if($mode == 'all')
	{
		if($val != '') {
			$where .= " AND ((Select vCompanyName from ".PRJ_DB_PREFIX."_organization_master where iOrganizationID=b2sa.iBuyer2Id) LIKE '%$val%'
						OR (Select vCompanyName from ".PRJ_DB_PREFIX."_organization_master where iOrganizationID=b2sa.iSupplierId) LIKE '%$val%' OR vACode LIKE '%$val%') ";
		}
		// echo $where; exit;
	}
	else if($mode == 'srch')
	{
      if(trim($buyer2) != '')
		...
		if(trim($code) != '')
		{
			$where .= " AND b2sa.vACode LIKE '%$code%'";
		}
      if(trim($supplier) != '')
		{
			$where .= " AND (Select vCompanyName from ".PRJ_DB_PREFIX."_organization_master where iOrganizationID=iSupplierId) LIKE '%$buyer%'";
		}
	}

	if($_SESSION['SESS_'.PRJ_CONST_PREFIX.'_USER_TYPE_SHORT'] == 'OA' && $uorg_type=='Buyer2') {
		$where .= " AND (b2sa.iBuyer2Id='".$_SESSION['SESS_'.PRJ_CONST_PREFIX.'_ORGID']."')";
	} else if($_SESSION['SESS_'.PRJ_CONST_PREFIX.'_USER_TYPE_SHORT'] == 'OA' && $uorg_type!='Buyer') {
		$where .= " AND (b2sa.iSupplierId='".$_SESSION['SESS_'.PRJ_CONST_PREFIX.'_ORGID']."')";
	}
