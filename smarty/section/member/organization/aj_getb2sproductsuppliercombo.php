...
if(trim($iBuyer2Id)!='' && $iBuyer2Id>0 && trim($iProductId)!='' && $iProductId>0) {
   $whr .= " AND iOrganizationID NOT IN (Select iSupplierId from ".PRJ_DB_PREFIX."_buyer2_supplier_sproduct_association where iBuyer2Id=$iBuyer2Id AND iProductId=$iProductId AND NOT (eStatus='Delete' AND eNeedToVerify='No'))";
}
if(trim($sid)!='' && $sid>0) {
   $whr .= " OR iOrganizationID=$sid ";
}
$whr .= " AND (eOrganizationType='Supplier' OR eOrganizationType='Both') AND iOrganizationID IN (Select iSupplierId from ".PRJ_DB_PREFIX."_buyer2_supplier_association where iBuyer2Id=$iBuyer2Id AND NOT (eStatus='Delete' AND eNeedToVerify='No') AND eStatus!='Need to Verify') ";
// echo $whr; exit;
