...
if(trim($iBuyer2Id)!='' && $iBuyer2Id>0) {
   $whr .= " AND iOrganizationID NOT IN (Select iBuyerId from ".PRJ_DB_PREFIX."_buyer2_buyer_association where iBuyer2Id=$iBuyer2Id AND NOT (eStatus='Delete' AND eNeedToVerify='No'))";
}
if(trim($sid)!='' && $sid>0) {
   $whr .= " OR iOrganizationID=$sid ";
}
$whr .= " AND (eOrganizationType='Buyer' || eOrganizationType='Both') ";
// echo $whr; exit;
