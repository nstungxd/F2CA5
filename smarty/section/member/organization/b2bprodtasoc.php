...
if(trim($iAssociationId)!='' && $iAssociationId>0) {
   $mod = 'edit';
   $flds = " b2bpa.*, org.vCompanyName as vBuyer2, org.vCompCode as vB2Code, bpo.vProductName as vProduct, bpo.vProductCode ";
   ...
   $arr = $b2bpaObj->getJoinTableInfo($jtbl, $flds, " AND iAssociationId=$iAssociationId ");
   if(!(is_array($arr) && count($arr)>0)) {
	...
	if($arr[0]['eStatus']=='Delete' && $arr[0]['eNeedToVerify']!='Yes') {
		header('Location: '.SITE_URL_DUM.'b2bprodtasoclist/rnme');
		exit;
	}
} else {
   $mod = 'add';
