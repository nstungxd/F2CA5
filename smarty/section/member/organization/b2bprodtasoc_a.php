...
if($mod=='verify')
{
   $dtv = $vasocdt = $b2bpavObj->getDetails('*', " AND iAssociationId=$iAssociationId ", ' iVerifiedID DESC ', '', ' LIMIT 0,1');
   $dtv = $dtv[0];
   if(is_array($vasocdt) && count($vasocdt)>0 && isset($vasocdt[0]['iAssociationId']) && isset($vasocdt[0]['iVerifiedID'])) {
      ...
      if($rs) {
         ...
         if($vasocdt[0]['eStatus']=='Delete' && $vasocdt[0]['eNeedToVerify']=='Yes') {
            $rs = $b2bpaObj->delete($iAssociationId);
         } else {
            $rs = $b2bpaObj->updateData($dtv, " iAssociationId=$iAssociationId ");
         }
      }
   } else {
...
else if(trim($iAssociationId)!='' && $iAssociationId>0 && $mod=='edit')
{
   ...
   $odtl = $b2bpaObj->select($iAssociationId);
	if(trim($odtl[0]['vACode'])=='') {
		$dtv['vACode'] = $Data['vACode'] = $b2bpaObj->getUniqueCode('');
	}
   ...
   // pr($Data); exit;
   $rs = $b2bpavObj->insert($Data);
   if($rs) {
      $r = $b2bpaObj->updateData($dtv, " iAssociationId=$iAssociationId ");
      $msg = 'rus';
		//
		$esubtyp = 'Buyer2 BProduct Association Modified';
...
else if($mod == 'add')
{
   // $mod = 'add';
	...
	$drec = $b2bpaObj->getDetails('*'," AND iBuyer2Id=".$Data['iBuyer2Id']." AND iProductId=".$Data['iProductId']."");
   ...
   $Data['eStatus'] = 'Need to Verify';
	$Data['vACode'] = $b2bpaObj->getUniqueCode('');
   $rs = $id = $b2bpaObj->insert($Data);
   if($rs) {
      $Data['iAssociationId'] = $rs;
      $r = $b2bpavObj->insert($Data);
      $msg = 'ras';
		//
		$esubtyp = 'New Buyer2 BProduct Association';
      $whr_cndt = " AND vType='$esubtyp' AND eSection='Member' " ;
      $email_cnt = $emailObj->getDetails('*', $whr_cndt);
      $type = "Create";
...
if(($mod=='add' || $mod=='edit') && $rs && $id>0)
{
	...
	$assocs = $b2bpaObj->getDetails($flds," AND b2bpa.iAssociationId=$id "); 	// AND b2bpa.eStatus IN ('Active','Inactive') AND b2bpa.eNeedToVerify!='Yes'
	// pr($assocs); exit;
	if(is_array($assocs) && count($assocs)>0 && isset($email_cnt) && is_array($email_cnt) && count($email_cnt)>0)
	{
		$verifyid = $b2bpavObj->getDetails('iVerifiedID'," AND iAssociationId='".$id."' ",' iVerifiedID DESC ','',' LIMIT 0,1 ');
		if(isset($verifyid[0]['iVerifiedID']) && $verifyid[0]['iVerifiedID']>0) {
			...
			$link = SITE_URL_DUM."b2bprodtasocview/".$id;
...
if(trim($admr)=='admr') {
   ...
   $_SESSION[PRJ_CONST_PREFIX.'_action_msg'] = $msg;
	header('Location: '.SITE_URL_DUM.'b2bprodtasoc/'.$msg);
} else {
   $_SESSION[PRJ_CONST_PREFIX.'_action_msg'] = $msg;
	header('Location: '.SITE_URL_DUM.'b2bprodtasoclist/'.$msg);
}
exit;
?>