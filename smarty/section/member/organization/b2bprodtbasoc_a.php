...
if($mod=='verify')
{
   ...
   if(is_array($vasocdt) && count($vasocdt)>0 && isset($vasocdt[0]['iAssociationId']) && isset($vasocdt[0]['iVerifiedID'])) {
      ...
      $rs = $b2bpbvObj->updateData($dtv, " iVerifiedID='".$vasocdt[0]['iVerifiedID']."'");
      if($rs) {
         unset($dtv['iVerifiedID']);
         unset($dtv['iAssociationId']);
         if($vasocdt[0]['eStatus']=='Delete' && $vasocdt[0]['eNeedToVerify']=='Yes') {
            $rs = $b2bpbObj->delete($iAssociationId);
         } else {
            $rs = $b2bpbObj->updateData($dtv, " iAssociationId=$iAssociationId ");
         }
      }
   } else {
      $dtv = $vasocdt = $b2bpbObj->select($iAssociationId);
      $dtv = $dtv[0];
      $dtv['eVerifiedBy'] = $sess_usertype_short;
      $dtv['iVerifiedByID'] = $sess_id;
      $dtv['dVerifiedDate'] = calcGTzTime(date('Y-m-d H:i:s'), 'Y-m-d H:i:s');
...
else if($mod=='reject')
{
   ...
   if(is_array($vasocdt) && count($vasocdt)>0 && isset($vasocdt[0]['iAssociationId']) && isset($vasocdt[0]['iVerifiedID'])) {
      ...
      if($rs) {
         // unset($dtv['iVerifiedID']);
         // unset($dtv['iAssociationId']);
         $rs = $b2bpbObj->updateData($dt, " iAssociationId=$iAssociationId ");
      }
   } else {
      $dtv = $vasocdt = $b2bpbObj->select($iAssociationId);
      $dtv = $dtv[0];
...
else if(trim($iAssociationId)!='' && $iAssociationId>0 && $mod=='edit')
{
   ...
   $odtl = $b2bpbObj->select($iAssociationId);
	if(trim($odtl[0]['vACode'])=='') {
		$dtv['vACode'] = $Data['vACode'] = $b2bpbObj->getUniqueCode('');
	}
   ...
   // pr($Data); exit;
   $rs = $b2bpbvObj->insert($Data);
   if($rs) {
      $r = $b2bpbObj->updateData($dtv, " iAssociationId=$iAssociationId ");
      $msg = 'rus';
		//
		$esubtyp = 'Buyer2 BProduct Buyer Association Modified';
...
else if($mod == 'add')
{
   ...
   if($rs) {
      ...
      $r = $b2bpbvObj->insert($Data);
      $msg = 'ras';
		//
		$esubtyp = 'New Buyer2 BProduct Buyer Association';
      $whr_cndt = " AND vType='$esubtyp' AND eSection='Member' " ;
      $email_cnt = $emailObj->getDetails('*', $whr_cndt);
      $type = "Create";
      $body = Array("#BUYER2#","#PRODUCT#","#BUYER#","#ACODE#","#LINK#","#ADDED_BY#");
		$body_arr = Array("#USER#","#BUYER2#","#PRODUCT#","#BUYER#","#ACODE#","#LINK#","#ADDED_BY#","#MAIL_FOOTER#","#SITE_URL#");
   } else {
      $msg = 'aerr';
   }
}
...
if(($mod=='add' || $mod=='edit') && $rs && $id>0)
{
	...
	$assocs = $b2bpbObj->getDetails($flds," AND b2bpb.eStatus IN ('Active','Inactive') AND b2bpb.eNeedToVerify!='Yes' AND b2bpb.iAssociationId=$id ");
	if(is_array($assocs) && count($assocs)>0 && isset($email_cnt) && is_array($email_cnt) && count($email_cnt)>0)
	{
		$verifyid = $b2bpbvObj->getDetails('iVerifiedID'," AND iAssociationId='".$id."' ",' iVerifiedID DESC ','',' LIMIT 0,1 ');
		if(isset($verifyid[0]['iVerifiedID']) && $verifyid[0]['iVerifiedID']>0) {
			...
			$link = SITE_URL_DUM."b2bprodtbasocview/".$id;
			$post = array($assocs[0]['vBuyer2'],$assocs[0]['vProduct'],$assocs[0]['vBuyer'],$assocs[0]['vACode'],$link,$sess_user_name."($sess_usertype_short)");
			...
			/*if(trim($smemails)!='') {
				$user = 'Security Manager';
				$post_arr = Array($user,$assocs[0]['vBuyer2'],$assocs[0]['vProduct'],$assocs[0]['vBuyer'],$assocs[0]['vACode'],$link,$sess_user_name."($sess_usertype_short)",$MAIL_FOOTER,SITE_URL_DUM);
				$sendMail->Send($esubtyp,"Member",$smemails,$body_arr,$post_arr);
			}
			if(trim($oaemails)!='') {
				$user = 'Organization Admin';
...
if(trim($admr)=='admr') {
   ...
   $_SESSION[PRJ_CONST_PREFIX.'_action_msg'] = $msg;
	header('Location: '.SITE_URL_DUM.'b2bprodtbasoc/'.$msg);
} else {
   $_SESSION[PRJ_CONST_PREFIX.'_action_msg'] = $msg;
	header('Location: '.SITE_URL_DUM.'b2bprdtbasoclist/'.$msg);
}
exit;
?>