...
if(trim($iAssociationId)!='' && $iAssociationId>0) {
   $mod = 'edit';
   $flds = " b2ba.*, org.vCompanyName as vBuyer2, bo.vCompanyName as vBuyer";
   $jtbl = " LEFT JOIN ".PRJ_DB_PREFIX."_organization_master bo on bo.iOrganizationID=b2ba.iBuyerId
               LEFT JOIN ".PRJ_DB_PREFIX."_organization_master org on org.iOrganizationID=b2ba.iBuyer2Id ";
   $arr = $b2baObj->getJoinTableInfo($jtbl, $flds, " AND b2ba.iAssociationId=$iAssociationId ");
	//	pr($arr); exit;
   if(!(is_array($arr) && count($arr)>0)) {
      ...
      header('Location: '.SITE_URL_DUM.'b2buyerasocview/'.$iAssociationId);
      exit;
   }
