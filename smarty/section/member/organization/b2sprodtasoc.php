...
if(trim($iAssociationId)!='' && $iAssociationId>0) {
   $mod = 'edit';
   $flds = " b2spa.*, org.vCompanyName as vBuyer2, org.vCompCode as vB2Code, bpo.vProductName as vProduct, bpo.vProductCode ";
   $jtbl = " LEFT JOIN ".PRJ_DB_PREFIX."_sproduct_organization bpo on bpo.iProductId=b2spa.iProductId
               LEFT JOIN ".PRJ_DB_PREFIX."_organization_master org on org.iOrganizationID=b2spa.iBuyer2Id ";
   $arr = $b2spaObj->getJoinTableInfo($jtbl, $flds, " AND iAssociationId=$iAssociationId ");
   if(!(is_array($arr) && count($arr)>0)) {
      ...
      header('Location: '.SITE_URL_DUM.'b2sprodtasocview/'.$iAssociationId);
      exit;
   }
