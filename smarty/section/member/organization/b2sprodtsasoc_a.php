...
if($mod=='verify')
{
   $dtv = $vasocdt = $b2spsvObj->getDetails('*', " AND iAssociationId=$iAssociationId ", ' iVerifiedID DESC ', '', ' LIMIT 0,1');
   $dtv = $dtv[0];
   if(is_array($vasocdt) && count($vasocdt)>0 && isset($vasocdt[0]['iAssociationId']) && isset($vasocdt[0]['iVerifiedID'])) {
      ...
      $rs = $b2spsvObj->updateData($dtv, " iVerifiedID='".$vasocdt[0]['iVerifiedID']."'");
      if($rs) {
         unset($dtv['iVerifiedID']);
         unset($dtv['iAssociationId']);
         if($vasocdt[0]['eStatus']=='Delete' && $vasocdt[0]['eNeedToVerify']=='Yes') {
            $rs = $b2spsObj->delete($iAssociationId);
         } else {
            $rs = $b2spsObj->updateData($dtv, " iAssociationId=$iAssociationId ");
         }
      }
   } else {
      $dtv = $vasocdt = $b2spsObj->select($iAssociationId);
      $dtv = $dtv[0];
      $dtv['eVerifiedBy'] = $sess_usertype_short;
      $dtv['iVerifiedByID'] = $sess_id;
      $dtv['dVerifiedDate'] =calcGTzTime(date('Y-m-d H:i:s'), 'Y-m-d H:i:s');
...
else if($mod=='reject')
{
   ...
   if(is_array($vasocdt) && count($vasocdt)>0 && isset($vasocdt[0]['iAssociationId']) && isset($vasocdt[0]['iVerifiedID'])) {
      ...
      if($rs) {
         // unset($dtv['iVerifiedID']);
         // unset($dtv['iAssociationId']);
         $rs = $b2spsObj->updateData($dt, " iAssociationId=$iAssociationId ");
      }
   } else {
      $dtv = $vasocdt = $b2spsObj->select($iAssociationId);
      $dtv = $dtv[0];
...
else if(trim($iAssociationId)!='' && $iAssociationId>0 && $mod=='edit')
{
   ...
   $odtl = $b2spsObj->select($iAssociationId);
	if(trim($odtl[0]['vACode'])=='') {
		$dtv['vACode'] = $Data['vACode'] = $b2spsObj->getUniqueCode('');
	}
   ...
   // pr($Data); exit;
   $rs = $b2spsvObj->insert($Data);
   if($rs) {
      $r = $b2spsObj->updateData($dtv, " iAssociationId=$iAssociationId ");
      $msg = 'rus';
		//
		$esubtyp = 'Buyer2 SProduct Supplier Association Modified';
...
else if($mod == 'add')
{
   // $mod = 'add';
	...
	$drec = $b2spsObj->getDetails('*'," AND iBuyer2Id=".$Data['iBuyer2Id']." AND iSupplierId=".$Data['iSupplierId']." AND iProductId=".$Data['iProductId']."");
   ...
   $Data['eStatus'] = 'Need to Verify';
	$Data['vACode'] = $b2spsObj->getUniqueCode('');
   $rs = $id = $b2spsObj->insert($Data);
   if($rs) {
      $Data['iAssociationId'] = $rs;
      $r = $b2spsvObj->insert($Data);
      $msg = 'ras';
		//
		$esubtyp = 'New Buyer2 SProduct Supplier Association';
      $whr_cndt = " AND vType='$esubtyp' AND eSection='Member' " ;
      $email_cnt = $emailObj->getDetails('*', $whr_cndt);
      $type = "Create";
      $body = Array("#BUYER2#","#PRODUCT#","#SUPPLIER#","#ACODE#","#LINK#","#ADDED_BY#");
		$body_arr = Array("#USER#","#BUYER2#","#PRODUCT#","#SUPPLIER#","#ACODE#","#LINK#","#ADDED_BY#","#MAIL_FOOTER#","#SITE_URL#");
   } else {
      $msg = 'aerr';
   }
}
...
if(($mod=='add' || $mod=='edit') && $rs && $id>0)
{
	...
	$assocs = $b2spsObj->getDetails($flds," AND b2sps.eStatus IN ('Active','Inactive') AND b2sps.eNeedToVerify!='Yes' AND b2sps.iAssociationId=$id ");
	if(is_array($assocs) && count($assocs)>0 && isset($email_cnt) && is_array($email_cnt) && count($email_cnt)>0)
	{
		$verifyid = $b2spsvObj->getDetails('iVerifiedID'," AND iAssociationId='".$id."' ",' iVerifiedID DESC ','',' LIMIT 0,1 ');
		if(isset($verifyid[0]['iVerifiedID']) && $verifyid[0]['iVerifiedID']>0) {
			...
			$link = SITE_URL_DUM."b2sprodtsasocview/".$id;
			$post = array($assocs[0]['vBuyer2'],$assocs[0]['vProduct'],$assocs[0]['vSupplier'],$assocs[0]['vACode'],$link,$sess_user_name."($sess_usertype_short)");
			...
			/*if(trim($smemails)!='') {
				$user = 'Security Manager';
				$post_arr = Array($user,$assocs[0]['vBuyer2'],$assocs[0]['vProduct'],$assocs[0]['vSupplier'],$assocs[0]['vACode'],$link,$sess_user_name."($sess_usertype_short)",$MAIL_FOOTER,SITE_URL_DUM);
				$sendMail->Send($esubtyp,"Member",$smemails,$body_arr,$post_arr);
			}
			if(trim($oaemails)!='') {
				$user = 'Organization Admin';
...
if(trim($admr)=='admr') {
   ...
   $_SESSION[PRJ_CONST_PREFIX.'_action_msg'] = $msg;
	header('Location: '.SITE_URL_DUM.'b2sprodtsasoc/'.$msg);
} else {
   $_SESSION[PRJ_CONST_PREFIX.'_action_msg'] = $msg;
	header('Location: '.SITE_URL_DUM.'b2sprdtsasoclist/'.$msg);
}
exit;
?>