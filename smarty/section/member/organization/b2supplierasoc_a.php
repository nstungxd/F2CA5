...
if($mod=='verify')
{
   $dtv = $vasocdt = $b2savObj->getDetails('*', " AND iAssociationId=$iAssociationId ", ' iVerifiedID DESC ', '', ' LIMIT 0,1');
   $dtv = $dtv[0];
   if(is_array($vasocdt) && count($vasocdt)>0 && isset($vasocdt[0]['iAssociationId']) && isset($vasocdt[0]['iVerifiedID'])) {
      ...
      $rs = $b2savObj->updateData($dtv, " iVerifiedID='".$vasocdt[0]['iVerifiedID']."'");
      if($rs) {
         unset($dtv['iVerifiedID']);
         unset($dtv['iAssociationId']);
         if($vasocdt[0]['eStatus']=='Delete' && $vasocdt[0]['eNeedToVerify']=='Yes') {
            $rs = $b2saObj->delete($iAssociationId);
         } else {
            $rs = $b2saObj->updateData($dtv, " iAssociationId=$iAssociationId ");
         }
      }
   } else {
      $dtv = $vasocdt = $b2saObj->select($iAssociationId);
      $dtv = $dtv[0];
      $dtv['eVerifiedBy'] = $sess_usertype_short;
      $dtv['iVerifiedByID'] = $sess_id;
      $dtv['dVerifiedDate'] =calcGTzTime(date('Y-m-d H:i:s'), 'Y-m-d H:i:s');
...
else if($mod=='reject')
{
   ...
   if(is_array($vasocdt) && count($vasocdt)>0 && isset($vasocdt[0]['iAssociationId']) && isset($vasocdt[0]['iVerifiedID'])) {
      ...
      if($rs) {
         // unset($dtv['iVerifiedID']);
         // unset($dtv['iAssociationId']);
         $rs = $b2saObj->updateData($dt, " iAssociationId=$iAssociationId ");
      }
   } else {
      $dtv = $vasocdt = $b2saObj->select($iAssociationId);
      $dtv = $dtv[0];
...
else if(trim($iAssociationId)!='' && $iAssociationId>0 && $mod=='edit')
{
   ...
   $odtl = $b2saObj->select($iAssociationId);
	if(trim($odtl[0]['vACode'])=='') {
		$dtv['vACode'] = $Data['vACode'] = $b2saObj->getUniqueCode('');
	}
   ...
   // pr($Data); exit;
   $rs = $b2savObj->insert($Data);
   if($rs) {
      $r = $b2saObj->updateData($dtv, " iAssociationId=$iAssociationId ");
      $msg = 'rus';
		//
		$esubtyp = 'Buyer2 Supplier Association Modified';
...
else if($mod == 'add')
{
   // $mod = 'add';
	...
	$drec = $b2saObj->getDetails('*'," AND iBuyer2Id=".$Data['iBuyer2Id']." AND iSupplierId=".$Data['iSupplierId']."");
   ...
   $Data['eStatus'] = 'Need to Verify';
	$Data['vACode'] = $b2saObj->getUniqueCode('');
   $rs = $id = $b2saObj->insert($Data);
   if($rs) {
      $Data['iAssociationId'] = $rs;
      $r = $b2savObj->insert($Data);
      $msg = 'ras';
		//
		$esubtyp = 'New Buyer2 Supplier Association';
      $whr_cndt = " AND vType='$esubtyp' AND eSection='Member' " ;
      $email_cnt = $emailObj->getDetails('*', $whr_cndt);
      $type = "Create";
      $body = Array("#BUYER2#","#SUPPLIER#","#ACODE#","#LINK#","#ADDED_BY#");
		$body_arr = Array("#USER#","#BUYER2#","#SUPPLIER#","#ACODE#","#LINK#","#ADDED_BY#","#MAIL_FOOTER#","#SITE_URL#");
   } else {
      $msg = 'aerr';
   }
}
...
if(($mod=='add' || $mod=='edit') && $rs && $id>0)
{
	...
	$assocs = $b2saObj->getDetails($flds," AND b2sa.eStatus IN ('Active','Inactive') AND b2sa.eNeedToVerify!='Yes' AND b2sa.iAssociationId=$id ");
	if(is_array($assocs) && count($assocs)>0 && isset($email_cnt) && is_array($email_cnt) && count($email_cnt)>0)
	{
		$verifyid = $b2savObj->getDetails('iVerifiedID'," AND iAssociationId='".$id."' ",' iVerifiedID DESC ','',' LIMIT 0,1 ');
		if(isset($verifyid[0]['iVerifiedID']) && $verifyid[0]['iVerifiedID']>0) {
			...
			$link = SITE_URL_DUM."b2supplierasocview/".$id;
			$post = array($assocs[0]['vBuyer2'],$assocs[0]['vSupplier'],$assocs[0]['vACode'],$link,$sess_user_name."($sess_usertype_short)");
			...
			/*if(trim($smemails)!='') {
				$user = 'Security Manager';
				$post_arr = Array($user,$assocs[0]['vBuyer2'],$assocs[0]['vSupplier'],$assocs[0]['vACode'],$link,$sess_user_name."($sess_usertype_short)",$MAIL_FOOTER,SITE_URL_DUM);
				$sendMail->Send($esubtyp,"Member",$smemails,$body_arr,$post_arr);
			}
			if(trim($oaemails)!='') {
				$user = 'Organization Admin';
...
if(trim($admr)=='admr') {
   ...
   $_SESSION[PRJ_CONST_PREFIX.'_action_msg'] = $msg;
	header('Location: '.SITE_URL_DUM.'b2supplierasoc/'.$msg);
} else {
   $_SESSION[PRJ_CONST_PREFIX.'_action_msg'] = $msg;
	header('Location: '.SITE_URL_DUM.'b2supplierasoclist/'.$msg);
}
exit;
?>