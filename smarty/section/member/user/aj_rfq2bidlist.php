...
if ($mode == 'srch') {
    ...
    // echo $where;
    if (trim($eStatus) != '') {
        if (strtolower(trim($eStatus)) == strtolower($smarty->get_template_vars('LBL_BEST'))) {
            $where .= " AND r2bd.eStatus LIKE 'current' ";
        } else {
...
$fields = " DISTINCT r2bd.*, rfq2.*, IF(rfq2.eFrom='Invoice',ioh.vInvoiceCode,poh.vPOCode) as vInvoiceCode, IF(rfq2.eFrom='Invoice',ioh.fAcceptedAmount,poh.fPOTotal) as fAcceptedAmount, IF(rfq2.eFrom='Invoice',ioh.vBuyerName,poh.vBuyerCompanyName) as vBuyerName, IF(rfq2.eFrom='Invoice',ioh.vSupplierName,poh.vSupplierName) as vSupplierName, sm.vStatus_" . LANG . " as status, r2bd.eStatus, r2bd.eSaved, r2bd.eDelete, org.vCompanyName as vBuyer2,
