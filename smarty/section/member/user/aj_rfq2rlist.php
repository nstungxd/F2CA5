...
if ($mode == 'srch') {
    ...
    if (trim($eStatus) != '') {
        ...
        } if (strtolower(trim($eStatus)) == strtolower($smarty->get_template_vars('LBL_DELETE'))) {
            $where .= " AND rfq2.eDelete='Yes' ";
        } else if (strtolower(trim($eStatus)) == strtolower($smarty->get_template_vars('LBL_VERIFIED'))) {
...
$where .= " AND (rfq2.iStatusID=$rsts || rfq2.eAuctionStatus LIKE 'Cancelled') AND rfq2.eSaved!='Yes' AND rfq2.eDelete!='Verified' ";
// echo $where; exit;
