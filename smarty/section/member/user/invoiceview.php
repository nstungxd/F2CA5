...
if ($uorg_type == 'Buyer2') {
    ...
    exit;
} else if ($invoiceData[0]['iBuyerOrganizationID'] == $curORGID && $invoiceData[0]['iStatusID'] < $isdtls && $invoiceData[0]['eCreateByBuyer'] != 'Yes') {
    header("Location: " . SITE_URL_DUM . "invoicelist/all");
    exit;
}/* else if ($invoiceData[0]['iBuyerOrganizationID'] == $curORGID && ($invoiceData[0]['eSaved'] == 'Yes' || $invoiceData[0]['iaStatusID'] == $rjtsts) && $invoiceData[0]['eCreateByBuyer'] == 'Yes') {
...
// if($invoiceData['iPurchaseOrderID'] > 0) {
...
} else if (($nxtstatus['iStatusID'] == $crtsts[0]['iStatusID'] || $nxtstatus['iStatusID'] == $acptsts[0]['iStatusID']) && $invoiceData['iaStatusID'] == 0 && $invoiceData['iBuyerOrganizationID'] == $curORGID) {
    // pr($ntacpt); exit;
    $nxtstatus['vStatusMsg'] = $smarty->get_template_vars('LBL_NEED_TO_ACCEPT');
    $act = 'y';
    $smarty->assign('act', $act);
    $ntacpt = 'y';
    $smarty->assign('ntacpt', $ntacpt);
    $permitted = 'Yes';
} else if ($nxtstatus['iStatusID'] == $vrfsts[0]['iStatusID'] && $invoiceData['iModifiedByID'] != $_SESSION['SESS_' . PRJ_CONST_PREFIX . '_ID'] && $invoiceData['iBuyerOrganizationID'] == $curORGID) {
    // check permission
    if (isset($upermits) && in_array($vrfsts[0]['iStatusID'], $upermits['inv'])) {
        $permitted = 'Yes';
    }
}
...
if ($invoiceData['iBuyerOrganizationID'] == $curORGID && ($invoiceData['eSaved'] == 'Yes' || $invoiceData['iaStatusID'] == $rjtsts) && $invoiceData['eCreateByBuyer'] == 'Yes' && $permitted!='Yes') {
//  header("Location: " . SITE_URL_DUM . "invoicecreate/$iInvoiceID");
//  exit;
	if(isset($upermits) && in_array($crtsts[0]['iStatusID'], $upermits['inv'])) {
		header("Location: " . SITE_URL_DUM . "invoicecreate/$iInvoiceID");
...
$smarty->assign('isusts', $isusts[0]['iStatusID']);
