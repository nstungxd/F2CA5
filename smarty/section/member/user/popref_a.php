...
if($rs>0) {
	if($eSaved=='Yes') {
		...
		$rs = $pohObj->updateData($dtl,$wh_cn);
                if($eFrom == "Next"){
                    $redirecturl = SITE_URL_DUM."purchaseorderadditems/$iPurchaseOrderID/sucss";
                }else{
   ...
   $redirecturl = SITE_URL_DUM."popref/$iPurchaseOrderID/rerr";
   $_SESSION['SESS_'.PRJ_CONST_PREFIX.'_MSG']="rerr";
}
...
if($view == '' || $view == 'add') {
   ...
   $Data['dOrderDate']=$dOrderDate;
	...
	if($id) {
		  ...
		  $msg = "ras";
		  $_SESSION['poadd']='yes';
		  $redirecturl = SITE_URL_DUM."purchaseorderadditems/$id/".$msg;
   } else {
		  $msg="raserr";
		  $redirecturl = SITE_URL_DUM.'polist/'.$msg;
   }
   ...
   $nstatus = $_POST['nstatus'];
	...
	if($id)
	{
		...
		if($nstatus == $isusts[0]['iStatusID'] && ($podtl[0]['iInvoiceID'] == '' || $podtl[0]['iInvoiceID']<1))
		{
			...
			$link = SITE_URL_DUM."purchaseorderview/".$iPurchaseOrderID;
			$body = Array("#POCODE#","#SUPPLIERORG#","#SUPORGCODE#","#BUYERORG#","#BUYORGCODE#","#LINK#");
			$post = Array($povdt[0]['vInvoiceCode'],$povdt[0]['vSupplierName'],$splrorg[0]['vOrganizationCode'],$povdt[0]['vBuyerCompanyName'],$povdt[0]['vBuyerCode'],$link);
			...
			$dt['iOrganizationID'] = $povdt[0]['iSupplierOrganizationID'];
			...
			if((is_array($tusrs) && count($tusrs) > 0)) {
				for($i=0;$i<count($tusrs);$i++) {
					...
					$post_arr = Array($smname,$sess_user_name."($sess_usertype_short)",$povdt[0]['vPOCode'],$povdt[0]['vSupplierName'],$splrorg[0]['vOrganizationCode'],$povdt[0]['vBuyerCompanyName'],$povdt[0]['vBuyerCode'],$link,$MAIL_FOOTER,SITE_URL_DUM);
...
//prints($stsdtls);exit;
	...
	if($id) {
            $files=$_FILES['files'];
                $titles=$_POST['titles'];
                $descprtiption=$_POST['descriptions'];
                $dAdate = calcGTzTime(date('Y-m-d H:i:s'), 'Y-m-d H:i:s');
                if(trim($files['name'][0]) == '')
                    unset($files['name'][0]);
		  ...
		  $msg = "rus";
		  $_SESSION['poadd']='yes';
		  $redirecturl = SITE_URL_DUM."purchaseorderadditems/$iPurchaseOrderID/".$msg;
   } else {
		  $msg="ruserr";
		  $redirecturl = SITE_URL_DUM.'polist/'.$msg;
   }
...
// mail
		if($in > 0)
		{
			...
			$link = SITE_URL_DUM."invoiceview/".$in;
...
//
		...
		// $attach = "";
			...
			$emailArr[0] = array('vFirstName'=>'name','vLastName'=>'name','vEmail'=>'pural.pandya@phptalent.com');
			// prints($usrarr); exit;
			if(is_array($emailArr) && count($emailArr) > 0) {
				for($i=0;$i<count($emailArr);$i++) {
					// echo $emailArr[$i]['vFirstName'].' '.$emailArr[$i]['vLastName']; exit;
					 ...
					 $post_arr = Array($smname,$sess_user_name."($sess_usertype_short)",$invdt[0]['vInvoiceCode'],$invdt[0]['vSupplierName'],$invdt[0]['vInvoiceSupplierCode'],$invdt[0]['vBuyerName'],$invdt[0]['vAssociatePOBuyerCode'],$link,$MAIL_FOOTER,SITE_URL_DUM);
			...
			if(is_array($usrarr) && count($usrarr) > 0) {
				for($i=0;$i<count($usrarr);$i++) {
					 ...
					 $post_arr = Array($smname,$SITE_NAME,$invdt[0]['vInvoiceCode'],$invdt[0]['vSupplierName'],$invdt[0]['vInvoiceSupplierCode'],$invdt[0]['vBuyerName'],$invdt[0]['vAssociatePOBuyerCode'],$link,$MAIL_FOOTER,SITE_URL_DUM);
...
//          if(is_array($emailArr) && count($emailArr) > 0) {
               ...
               if($podt[0]['iStatusID'] != $isusts[0]['iStatusID']) {
					...
					if($podt[0]['iInvoiceID']>0) {
						...
						$mchpref = $orgprefObj->getStatusDetails($poData[0]['iBuyerOrganizationID']);
						$mth = array();
						...
						$orgpostatus = $orgpref['po'];
						for($l=0; $l<count($orgpostatus); $l++) {
							if($orgpostatus[$l]['iStatusID'] > $poData[0]['iStatusID'] && in_array($orgpostatus[$l]['iStatusID'],$mth)) {
								$nxtstatus = $orgpostatus[$l];
								break;
							}
						}
               ...
               $link = SITE_URL_DUM."purchaseorderview/".$id;
               ...
               $post = Array($sess_user_name."($sess_usertype_short)",$podt[0]['vPOCode'],$podt[0]['vSupplierName'],$supdt[0]['vOrganizationCode'],$podt[0]['vBuyerCompanyName'],$podt[0]['vBuyerCode'],$link);
               ...
               $dt['iOrganizationID'] = $podt[0]['iBuyerOrganizationID'];
               $dt['vMailSubject_en'] = $db_email[0]['vSub_en'];
               $dt['vMailSubject_fr'] = $db_email[0]['vSub_fr'];
               $dt['tMailContent_en'] = $emailContent_en;
               $dt['tMailContent_fr'] = $emailContent_fr;
               ...
               if((is_array($emailArr) && count($emailArr) > 0)) {
						...
						$smname = $emailArr[$i]['vFirstName'].' '.$emailArr[$i]['vLastName'];
						$email = $emailArr[$i]['vEmail'];
							  $post_arr = Array($smname,$sess_user_name."($sess_usertype_short)",$podt[0]['vPOCode'],$podt[0]['vSupplierName'],$supdt[0]['vOrganizationCode'],$podt[0]['vBuyerCompanyName'],$podt[0]['vBuyerCode'],$link,$MAIL_FOOTER,SITE_URL_DUM);
							  $sendMail->Send($sub1,"Member",$email,$body_arr,$post_arr);
						}
					}
					if((is_array($usrarr) && count($usrarr) > 0)) {
