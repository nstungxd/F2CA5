...
//prints($poData[0]);
//exit;
/* if((($poData[0]['iStatusID'] == $rjtsts && $poData[0]['eDelete'] != 'Yes' && $poData[0]['iSupplierOrganizationID']!=$curORGID) || $poData[0]['eSaved']=='Yes') && $sess_usertype_short == 'OU') {
  if(!($poData[0]['iSupplierOrganizationID']==$curORGID && $poData[0]['iaStatusID']==$rjtsts)) {
  if($poData[0]['iBuyerOrganizationID']==$curORGID) {
        ...
        if ($poData['iStatusID'] != $isusts[0]['iStatusID'] && $poData['iStatusID'] != $acptsts[0]['iStatusID']) {  // count($orgusrs) > 1 &&
            if ($poData['iStatusID'] == $orgpostatus[count($orgpostatus) - 1]['iStatusID'] && $poData['iStatusID'] != $rjtsts) {
              ...
              $nxtstatus = $orgpostatus[1];
                ...
                $nxtset = 'n';
                // $nxtstatus = (isset($orgpostatus[$l]))? $orgpostatus[$l] : '';
                $nxtstatus = $orgpostatus[0];
...
if (isset($nxtstatus['vStatus']) && $nxtstatus['vStatus'] != '') {
    if ($nxtstatus['vStatus'] == 'Auth1' || $nxtstatus['vStatus'] == 'Auth2' || $nxtstatus['vStatus'] == 'Auth3' || (isset($nxtstatus['vStatus_en']) && ($nxtstatus['vStatus_en'] == 'Auth1' || $nxtstatus['vStatus_en'] == 'Auth2' || $nxtstatus['vStatus_en'] == 'Auth3'))) {
        $auth = 'y';
    }
...
} else if ($nxtstatus['iStatusID'] == $crtsts[0]['iStatusID'] && $poData[0]['iaStatusID'] == 0 && $poData['iSupplierOrganizationID'] == $curORGID) {
    $nxtstatus['vStatusMsg'] = $smarty->get_template_vars('LBL_NEED_TO_ACCEPT');
    $act = 'y';
    $smarty->assign('act', $act);
}

if ((($poData['iStatusID'] == $rjtsts && $poData['eDelete'] != 'Yes' && $poData['iSupplierOrganizationID'] != $curORGID) || $poData['eSaved'] == 'Yes') && $sess_usertype_short == 'OU') {
