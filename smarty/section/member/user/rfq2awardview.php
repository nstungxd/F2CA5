	...
	if($b2rfq2sts=='completed')
	{
		...
		$jtbl = "INNER JOIN ".PRJ_DB_PREFIX."_rfq2_master rfq2 on r2bd.iRFQ2Id=rfq2.iRFQ2Id
            LEFT JOIN ".PRJ_DB_PREFIX."_inovice_order_heading ih ON rfq2.iInvoiceID=ih.iInvoiceID
				...
				LEFT JOIN ".PRJ_DB_PREFIX."_organization_master org ON org.iOrganizationID=r2bd.iBuyer2Id ";
		$where = " AND r2bd.iRFQ2Id=".$bdtls[0]['iRFQ2Id']." AND r2bd.iBidId NOT IN (Select iBidId from ".PRJ_DB_PREFIX."_rfq2award_master where iStatusID=$rsts) AND iBidId!=".$bdtls[0]['iBidId']." AND r2bd.eSaved!='Yes' AND r2bd.eStatus Not IN ('pending','rejected') "; 	// AND r2bd.iBidId!=".$bdtls[0]['iBidId']."
...
if(($bdtls[0]['iStatusID']<=$asts && $bdtls[0]['eStatus']!='Rejected') && $bdtls[0]['eSaved']!='Yes' && $b2rfq2sts!='cancelled') { 	// && $orgprf[0]['eRFQ2VerifyReq']=='Yes'
   $vreq = 'y';
	...
	if($bdtls[0]['iStatusID'] == $asts && $bdtls[0]['iOrganizationID']==$curORGID) { $vreq = 'n'; }
}
if($permitted=='y') {
