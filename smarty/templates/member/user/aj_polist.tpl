<table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr class="{$rowclass}">
        ...
        <td width="58" align="center">
              <b>
						...
						{else}
                        	...
                        	{$LBL_DELETE}
       						{elseif $activegroup[i].iSupplierOrganizationID eq $curORGID && $activegroup[i].iStatusID eq $isusts && $activegroup[i].iaStatusID eq 0}
		                  	{$LBL_NEED_TO_ACCEPT}
                        {else}
        ...
        <td width="74" align="center">
            ...
            {/if}
				{if $activegroup[i].status|htmlentities eq 'Accepted'}
					<a title="{$LBL_PRINT}" style="cursor:pointer" class="colorboxfile" rel="{$activegroup[i].iPurchaseOrderID}">
					<img src="{$SITE_IMAGES}report.png"  alt="" border="0" style="vertical-align:middle;" /></a> &nbsp;
					</a>
				{/if}
            {if $usertype neq 'orgadmin'}
